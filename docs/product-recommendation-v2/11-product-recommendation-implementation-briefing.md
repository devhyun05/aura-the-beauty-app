...
## 15. 이번에 구현하지 않은 2차 후보 (역사 기록, superseded)

> **SUPERSEDED — 2026-07-20:** 아래 미구현 판단은 이 브리핑 작성 당시의 역사 기록이다.
> 현재 첫 shelf와 추천 품질 계약은
> [13. 메이크업 추천 보고서 기반 제품 추천 정본](13-makeup-report-product-recommendations.md)을 따른다.

~~`AR 필터 기반 추천제품`을 `메이크업 추천 리포트 기반 추천제품`으로 바꾸는 작업은 이번 범위에서 구현하지 않았다. 메이크업 추천 리포트의 구조화 데이터 계약과 품질 기준이 아직 확정되지 않았기 때문이다.~~

현재는 메이크업 추천 보고서와 선택한 룩을 기준으로 첫 shelf를 제공한다. 기존 AR section API와
저장 룩·프리뷰 경로는 과거 클라이언트와 AR 기능 호환성을 위해 유지한다.
...
기존 AR section API와 저장 룩 계약은 현재도 호환성을 유지하지만, 추천제품 허브 첫 shelf의
추천 기준은 메이크업 추천 보고서로 대체되었다.
