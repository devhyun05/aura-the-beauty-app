...
-- Keep shared environments production-like: catalog categories remain, while
-- demo consultants and membership offers must not be visible at runtime.
delete from consulting_membership_plans
where id in ('plan_lite', 'plan_standard', 'plan_premium');

delete from consulting_experts
where id in ('exp_sea', 'exp_doa', 'exp_lian');
