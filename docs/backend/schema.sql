...
create table if not exists external_product_likes (
  ...
  constraint chk_external_product_likes_source check (external_source in ('naver_shopping_search')),
...
create table if not exists product_engagement_events (
  ...
  constraint chk_product_engagement_source check (
    (event_type = 'search_submit' and search_request_id is not null)
    or (event_type <> 'search_submit' and product_id is not null)
  )
);
