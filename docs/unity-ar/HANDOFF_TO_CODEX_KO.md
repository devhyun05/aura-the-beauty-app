...
### Unity MCP 토큰 수칙 (0723 실측 기반)
비용의 실체: MCP 호출 자체는 쌈(JSON 몇 줄). 비싼 건 ①이미지 열람 ②셋업 코드 재전송 ③시행착오 왕복.
1. **셋업은 에디터 스크립트로 박제, MCP는 한 줄 호출** — execute_code로 매번 수십 줄 보내지 말 것(타이밍 실수까지 유발). `GlamTestRig.Setup()/Capture()` 사용(반복당 비용 1/20 실측)
2. **이미지는 저장만, 열람은 합본 시트 1장** — 후보 N개 각각 열면 N배. 파이썬으로 한 장에 합쳐 한 번만 볼 것
3. **판정은 숫자 먼저** — luma로 깜깜한 캡처 걸러내기, 픽셀 diff로 "수정이 실제 반영됐나" 확인(v9 무변화를 270px로 적발). 이미지 열람 전에 무효 처방을 거른다
4. **텍스처·수치 반복은 파이썬, Unity 장착은 체크포인트만** — 평면 미리보기 통과분만 장착
5. **컴파일 왕복 절약** — 값 비교는 상수 수정·재컴파일 대신 머티리얼 프로퍼티 SetFloat 스윕(한 플레이 세션에 N값 캡처). 셰이더 수정 시 ImportAsset(ForceUpdate) 필수(안 하면 미반영 판정으로 왕복 증가)

한 줄 요약: **MCP는 리모컨으로만, 무거운 것(셋업·반복·판정)은 코드와 숫자에게.**
