...
`_BrowCleanupStrength`와 `_BrowNeutralizeStrength` 두 셰이더 변수는 선언만 있고 프래그먼트에서 읽힌 적이 없었으며, **2026-07-17에 선언과 세터를 모두 제거했습니다**(b9f9a023). 실제 눈썹 덮기 불투명도는 E3가 넣는 **하드코딩 `0.92`** 가 결정합니다.
