...
독자에 따라 읽을 곳이 다릅니다. 처음부터 끝까지 읽을 필요는 없습니다.

| 당신이 | 읽을 곳 |
|---|---|
| AR 메이크업이 처음이다 | **1장**만 읽으세요. 5분이면 전체 그림이 잡힙니다. |
| 지금 이 코드를 고치러 왔다 | **2장**부터 읽으세요. 현재 고장나 있거나 오해하기 쉬운 지점이 모여 있습니다. |
| 화장이 이상하게 나온다 | **15장** 진단 순서 → 해당 기술 장으로. |
| 구조를 판단해야 한다 | 2장 → 3장 → 16장. |
| 특정 부위만 알고 싶다 | 12장(부위별 차이) → 13장(보호 계층). |

4~11장은 참조용 기술 상세입니다. 필요할 때 찾아보면 됩니다.
...
**용어.** `blending`은 **블렌딩(합성)**, `mask`는 **마스크**로 표기합니다.
...
| 기술 용어 | 쉬운 의미 | 실제 역할 |
|---|---|---|
| 얼굴 mesh | 얼굴 모양의 입체 마네킹 | 추적된 얼굴 위치와 표면을 제공한다 |
| UV | 마네킹을 평면으로 펼친 지도 좌표 | 얼굴이 움직여도 같은 화장 위치를 찾는다 |
| 마스크(mask) | 화장할 곳만 뚫어 놓은 스텐실 도안 | 어느 픽셀에 얼마나 바를지 정한다 |
| texture | GPU가 읽는 이미지/데이터 판 | 마스크·그림·광택 지도를 GPU 메모리에 담는다 |
| shader | 픽셀별 화장 공식 | 원본색·제품색·마스크·마감을 계산한다 |
| material | 셰이더에 값을 채운 작업 지시서 | 같은 셰이더라도 립/블러셔별 색과 강도를 다르게 만든다 |
| 블렌딩 | 투명 필름을 겹치는 규칙 | 기존 카메라색과 새 화장색을 합친다 |
| renderer | 실제로 mesh와 material을 그리는 실행자 | 렌더 순서, 표시 여부, mesh, material을 관리한다 |

**가장 자주 오해하는 지점: 마스크와 texture는 같은 말이 아닙니다.** 마스크는 "어디에 적용할지"라는 *의미*이고, texture는 그 정보를 GPU에 담는 *그릇*입니다. 하나의 마스크는 PNG `Texture2D`, 런타임 생성 `R8 Texture2D`, RGBA32 다채널 `Texture2D` 중 무엇으로도 구현될 수 있습니다.
...
`_BrowCleanupStrength`와 `_BrowNeutralizeStrength` 두 uniform은 셰이더에 **선언만 되어 있고 프래그먼트 계산에서 단 한 번도 읽히지 않습니다.** 실제 눈썹 덮기 불투명도는 E3가 넣는 **하드코딩 `0.92`** 가 결정합니다.
...
- 근거(선언만 존재): `SmoothRegionMask.shader` — 프로퍼티 59–60, uniform 197–198. 셰이더 전체에서 이 4곳이 유일한 등장.
...
| `multiply` | `DstColor` | `Zero` | 필터색 × destination |
...
**연관 사실:** 파운데이션은 `multiply`를 요청해도 normal alpha blend를 유지합니다(`if (!foundationRegion)` 가드, 5239). 브로우도 강제로 `SrcAlpha/OneMinusSrcAlpha`가 됩니다 — 어두운 자연 눈썹을 밝은 피부색으로 덮어야 하는데 multiply로는 불가능하기 때문입니다(4515–4543).
...
| ARwithFable 립·브로우 | `Occlusion.cginc`의 `OccludeGate()` | SelfieMulticlass 세그멘테이션의 face-skin 확률 |
...
- 근거(E7 미배선): `MediaPipeGraft` 트리 전체에 `HandOcclusion` 문자열 0건. `Lip.shader`/`Brow.shader`에 `_HandOcclusion*` uniform 없음.
- 근거(대체 게이트): `Lip.shader` 84·226, `Brow.shader` 40·94가 `Occlusion.cginc`를 include하고 `OccludeGate(i.grabPos)`를 alpha에 곱함. `Occlusion.cginc` (66–71).
- 근거(실패 시 무효): `SegmentationSource.cs` (37, 97) — 실패 경로에서 gate=1.
...
정적 판정: **`applyFilter`가 `ARFilterScreen`에서 버려지는 것은 정적으로 확정(CONFIRMED). 다만 E3가 대신 렌더링하므로 실제 기기 증상은 `UNVERIFIED`.**
...
생성 마스크의 byte 수 검증(`width × height × 4`)은 존재하지만, **립과 브로우의 실패 처리가 비대칭**입니다.

| 경로 | 등록 실패 시 | 결과 |
|---|---|---|
| 브로우 | 반환값을 확인하고 예외를 던짐 | 거부됨 ✅ |
| 립 | **반환값을 버림** | 텍스처 없이 레이어가 그대로 적용됨 ⚠️ |

크기가 틀린 립 마스크는 경고만 남기고 텍스처 등록을 건너뛴 뒤, 레이어 구성과 적용은 그대로 진행됩니다.

- 근거(검증 위치): `E3RegionMaskOverlay` — 립 478–487, 브로우 536–545. 예외가 아니라 `Debug.LogWarning` + `return false`인 **약한** 검사.
- 근거(비대칭): `RNBridge` — 립 858은 반환값 미사용, 브로우 982–990은 `throw new InvalidOperationException`.
...
목·파운데이션의 정교한 semantic segmentation 경로는 코드에 존재하지만 현재 동작하지 않습니다. 그런데 **이유가 흔히 알려진 것과 다릅니다.**
...
| 생성 브로우 마스크의 **A 채널** | 생성기는 `desiredAlpha`를 기록하지만 셰이더는 `mask.a`를 **어디서도 읽지 않음** | `browGenerateCore.ts` 594 vs 셰이더 전체 |
| 렌더 큐 **3002·3003** | 어떤 상수에도 배정되지 않은 빈 번호 | `MakeupQueues.cs` |
| `E3RegionMaskOverlay` 4521 주석 | *"레시피가 브로우를 multiply로 보낸다"* — **낡음**. 계약은 이미 `normal`을 보냄 | vs `fullFaceMakeupRecipe.ts:1089` |
...
**두 경로가 나뉜 진짜 이유는 설계가 아니라 역사입니다.** 처음부터 하나로 설계된 엔진이 아니라, 서로 다른 기능을 위해 개발된 구현이 나중에 한 앱 안에서 합쳐졌습니다. 현재는 전환기라 **일부 립은 E3 요청을 받고도 ARwithFable renderer로 그려집니다.**
...
flowchart TD
    ...
    UI -->|개인화 립/브로우| GP[Generated raw RGBA payload]
...
- `MakeupController.OnMessage()`가 `applyFilter`를 받아 `ApplyTo()`로 material uniform과 부위별 renderer 값을 갱신합니다.
- 얼굴의 넓은 영역은 `FaceMakeup.shader`, 립·눈·눈썹 등은 전용 mesh와 전용 shader로 그립니다.
...
### 3.4 경로 B: 풀페이스 recipe와 개인화 UV 마스크
...
- 개인화 립/브로우는 별도 메시지로 raw RGBA 마스크를 등록한 뒤 같은 E3 overlay에서 렌더합니다.
- **pilot 예외:** 레시피의 립은 `LipRenderer.Instance`가 존재하면 ARwithFable 립 renderer로 우회하고 E3 립 overlay를 숨깁니다.
...
|---|---|---|
| 얼굴 기준 | MediaPipe 478 landmarks (얼굴 468 + 홍채 10) | AR Foundation `ARFace` vertices/indices/uvs |
| 넓은 영역 | `FaceMakeup.shader`가 GrabPass로 카메라를 읽고 자체 합성 | `SmoothRegionMask.shader`가 투명/곱셈 필터 출력 |
| 립 | 실제 립 외곽·내곽 landmark로 만든 ring mesh | 정적/생성 UV 마스크 mesh (단, 레시피는 pilot로 ARwithFable 우회 가능) |
| 마스크 생성 | Resources PNG 우선, 없으면 256×256 R8 절차 생성 | Resources PNG 또는 raw RGBA32 런타임 등록 |
| renderer 구성 | canonical face + 부위별 전용 renderer 다수 | 얼굴별·부위별 `E3 Region` dynamic mesh renderer |
| 최종 합성 | 전용 queue 3000~4000, 여러 shader | runtime queue 5000, sorting order 120 |
| 가림 방식 | 세그멘테이션 `OccludeGate` | E7 손 관절 마스크 + skin gate |
...
3D 얼굴 mesh의 각 vertex에는 `(x, y, z)` 위치뿐 아니라 `(u, v)` 좌표가 있습니다. UV는 보통 `0..1` 범위의 2D 좌표입니다.

- `u=0`은 텍스처 왼쪽, `u=1`은 오른쪽입니다.
- `v=0`과 `v=1`의 상하 방향은 소스와 업로드 방식에 따라 뒤집힐 수 있어 **코드 계약이 중요합니다.**
- 얼굴이 회전하거나 표정이 변해도 같은 vertex의 UV는 같은 얼굴 부위를 가리킵니다.

즉 "입술은 화면의 이 픽셀"이라고 저장하지 않고 **"얼굴 지도에서 이 영역"** 이라고 저장합니다. 그래서 고개를 돌려도 화장이 얼굴에 붙어 움직입니다.

### 4.2 ARwithFable의 canonical UV

`FaceLandmarkSource`는 MediaPipe 478개 landmark를 다루며, 얼굴 mesh는 468개, 나머지 10개는 양쪽 홍채 점입니다. `CanonicalFaceMesh`는 고정 topology와 UV를 읽고 매 프레임 landmark 위치만 새로 반영합니다.

- topology/UV: 고정
- vertex의 화면/월드 위치: 매 프레임 변경
...
1. UV와 texture가 사용 가능한지 검사
2. 추적 노이즈를 줄이기 위해 vertex별 motion-adaptive EMA 적용
3. 필요하면 마스크가 없는 triangle을 미리 제거
4. `face.uvs`를 새 dynamic mesh의 UV0에 복사
5. 완성한 mesh를 해당 얼굴 transform의 자식 renderer로 그림

**블러셔 일부는 예외입니다.** 특정 cheek 마스크는 ARFace UV 대신 얼굴 local `x/y`를 정규화한 좌표를 새로 만들어 양볼 배치를 맞춥니다.

근거: `E3RegionMaskOverlay.TryUpdateFullFaceUvMesh` (1731–2003) · cheek 예외 (3528–3572).

### 4.4 마스크는 항상 단일 흑백 채널이 아니다

채널 해석은 renderer마다 다릅니다.

| 마스크 유형 | 채널 의미 |
|---|---|
| ARwithFable 절차 마스크 | R = 적용 강도. R8로 생성 |
| E3 일반 립 atlas | R = 전체/기본 coverage, G = overline 계열, B = gradient·내부 density |
| E3 생성 브로우 | R = 자연 눈썹 neutralize footprint, G = 그릴 눈썹 body, B = 털 strand detail, **A = 미사용(F8)** |
| E3 cheek PNG | RGB luminance를 반전한 strength + 별도 blur와 중심/외곽 band |
| E3 렌즈 | Unity가 눈 영역에 맞춰 런타임 생성한 radial profile (pupil/rim 의미) |

**"PNG니까 알파를 읽겠지"라고 가정하면 안 됩니다.** 실제 shader가 어떤 채널을 읽는지가 계약의 진실입니다.

---

## 5. 마스크가 GPU texture가 되는 과정
...
절차 마스크는 `TextureFormat.R8`, `Clamp`, `Bilinear`이며, 픽셀값은 여러 ellipse 중 **가장 큰** coverage를 선택합니다. 블러셔는 Gaussian tail을 사용해 경계가 멀리까지 부드럽게 감소합니다. Softness는 **5개 bucket**으로 양자화하고 bucket별로 캐시하므로 slider가 움직일 때 매 프레임 새 texture를 만들지 않습니다.

> ⚠️ **F6 참조:** softness는 절차 생성에만 적용됩니다. `blush.png`·`lips.png`가 존재하므로 해당 부위 슬라이더는 무효입니다.

근거: `MaskGenerator` — `PlatformFolder` (129–135), `LoadOrGenerate` (200–206), `Generate` (210–214), MAX coverage (247), Gaussian tail (233–240), `SoftnessBuckets = 5` (141), bucket 캐시 (187–193).
...
개인화 립/브로우 생성 결과는 PNG 경로가 아니라 JSON 안의 `maskRawRgbaBase64`로 전달될 수 있습니다.
...
| 브로우 | 있음 | Trilinear | `SetPixelData` + `Apply(true,…)` | 미세한 털 결이 원거리에서 떨리지 않게 |

근거: `RNBridge` — 립 793–920 (검증 811–845), 브로우 922–1050 (거부 982–990) · `E3RegionMaskOverlay` — byte 검증 478–487/536–545, 립 등록 489–494, 브로우 등록 555–562.

512×512 RGBA32 마스크 한 장의 데이터 크기(검증됨):

- raw: 1,048,576 bytes = **1.00 MiB**
- Base64 본문: 약 **1.33 MiB**
- 브로우 mip chain: 이론상 원본의 4/3 = 약 **1.33 MiB**
...
## 6. texture와 material의 연결

### 6.1 Shader와 Material은 다르다

- **Shader**는 GPU가 실행할 프로그램입니다.
- **Material**은 그 shader를 선택하고 `_MaskTex`, `_RegionColor`, `_Opacity` 같은 실제 값을 채운 인스턴스입니다.

같은 `SmoothRegionMask.shader`를 써도 립 material과 브로우 material의 마스크·색·threshold·blend state는 서로 다를 수 있습니다.

### 6.2 ARwithFable material 구성

`MakeupController.CreateMaterial()`이 `FaceMakeup` shader material을 만들고 초기 마스크들을 연결합니다. `ApplyTo()`는 UI에서 넘어온 `FilterParams`를 이렇게 분배합니다.

| 대상 | 담당 |
|---|---|
| `FaceMakeup` material | smoothing, brightening, foundation, blush, highlight, contour, overlay |
| `LipRenderer` | lip color, intensity, finish, gradient, base lip, gloss |
| `BrowRenderer`, `PencilRenderer`, `StyleRenderer` | brow 제품 스택 |
| `IrisRenderer` | lens, eyeshadow, eyeliner |
| `LowerLidRenderer`, `AegyoRenderer`, `LashRenderer` | 눈 아래와 속눈썹 |
| `FaceWarpField` | 눈 확대, 턱·광대 등 얼굴형 warp |

**하나의 `FilterParams`가 하나의 거대한 shader에 전부 들어가는 게 아닙니다.** 넓은 얼굴 surface와 정밀 landmark 부위로 책임이 분산됩니다.

근거: `MakeupController.CreateMaterial` (185–215) · `ApplyTo` (878–1140).

### 6.3 E3 material 구성

E3는 얼굴별·부위별 `RegionOverlayView`를 만들고 `SmoothRegionMaskMaterial` template을 clone합니다. Template이 없으면 `MakeupAR/SmoothRegionMask` shader로 material을 직접 만듭니다.

`ApplyRecipeAppearance()`의 핵심 작업:

1. `_MaskTex`와 선택적 `_GlossMaskTex` 연결
2. `blendMode`를 GPU blend factor로 변환
3. `_RegionColor`, `_SecondaryColor`, `_Opacity` 설정
4. `_Threshold`, `_Feather`, UV offset/spread 설정
5. coverage, roughness, specular, gloss, detail 설정
6. foundation, cheek, eyeliner, brow, lens mode flag 설정
7. hand occlusion, skin gate, half-face flag 설정

근거: `E3RegionMaskOverlay.ApplyRecipeAppearance` (4284–4793) · material 생성 (5088–5270).

---

## 7. shader가 픽셀을 만드는 방식
...
이 shader는 일반 투명 색판과 **다르게** 동작합니다.

1. `GrabPass { "_CameraFeed" }`로 메이크업 전 카메라색을 읽습니다.
2. 얼굴 mesh의 각 fragment에서 원본 카메라 픽셀을 샘플합니다.
3. edge-preserving smoothing과 brightening을 적용합니다.
4. foundation, blush, highlight, contour, concealer, powder를 shader 내부에서 순차 합성합니다.
5. 최종 RGB를 alpha 1로 출력합니다.

그래서 effect가 0인 곳은 원본 카메라색을 그대로 다시 쓰며 mesh 경계가 보이지 않습니다. GPU blend state로 반투명 판을 얹는 게 아니라, **shader 내부에서 완성 픽셀을 재구성한 뒤 그 픽셀로 교체**하는 방식입니다.

블러셔 예시는 `tex2D(_BlushMask, buv).r × _BlushIntensity`를 coverage로 쓰고 `TintFinish()`가 원본 luma와 마감값을 반영합니다.

근거: `FaceMakeup.shader` (1–10, 155–170, 423–560).

### 7.3 ARwithFable 립 shader

립은 정적 UV 타원 마스크보다 **실제 외곽·내곽 landmark로 만든 ring mesh가 주 경계**입니다.

- 바깥 경계에서만 feather를 적용합니다.
- 카메라의 실제 입술 luma를 읽어 제품색의 명암을 보존합니다.
- base lip, tint, finish, gloss, particle을 순서대로 내부 합성합니다.
- 최종 출력은 **premultiplied** color입니다.
- blend state는 `Blend One OneMinusSrcAlpha`입니다.

이 방식은 평평한 단색 스티커보다 실제 입술 주름과 빛을 더 잘 보존하고, 글로스가 alpha를 키우지 않아도 빛을 더할 수 있습니다.

근거: `LipRenderer.cs` (9–27, 220–276) · `Lip.shader` (69–88, 149–227).

### 7.4 E3 `SmoothRegionMask.shader`

**첫 pass** — 이름은 `PigmentMultiplyOrAlphaFallback`, `ZWrite Off` / `ZTest Always` / `Cull Off`:

1. half-face 모드면 반대쪽 UV fragment를 `discard`합니다.
2. 기본은 face UV, 특별한 경우 clip position으로 screen UV를 만듭니다.
3. spread와 offset으로 마스크 UV를 보정합니다.
4. `_MaskTex`를 샘플합니다.
5. **13개 tap**(중심 1 + 축 4 + 대각 4 + 먼 축 4)으로 soft mask를 만듭니다.
6. `smoothstep` 기반 soft/core coverage를 계산합니다.
7. region mode에 따라 foundation, cheek, eyeliner, lens, generated brow, generic pigment 분기로 갑니다.
8. hand occlusion, tracking visibility, skin gate를 곱합니다.
9. material의 blend state에 맞는 RGB/alpha를 출력합니다.

> 13 tap은 `SampleMaskSoft` 기준입니다. 셰이더에는 같은 배치의 `GradientDensityBlur`(13 tap)와 `CheekSourceGrayBlur`(9 tap)도 있습니다.

**두 번째 pass** — 이름은 `GlossAdditiveHighlight`, 립 gloss 전용 additive pass, `Blend One One`. 조건이 안 맞으면 0을 반환합니다. `_LipStyleMode` 기본값이 `-1`이고 립 계열 마스크에만 실제 모드가 설정되므로, 립이 아닌 모든 region은 zero early-out으로 빠집니다.
...
alpha 0을 반환하는 것으로는 부족합니다. **multiply 블렌드에서는 alpha가 무시되므로 RGB가 검정이면 destination을 검게 만듭니다.** 셰이더 주석이 이걸 명시합니다:

> *"discard skips the pixel entirely so the excluded half shows the bare camera face under ANY blend mode. An alpha-0 return still paints BLACK where the region uses a multiply/opaque blend (lip, cheek), which is the bug."*

gloss pass는 이유가 하나 더 있습니다 — `Blend One One`으로 **빛을 더하고** alpha 0을 반환하도록 설계돼 있어서, alpha를 곱해도 억제가 안 됩니다. 반드시 early-out해야 합니다.

또한 이 게이트는 `_MaskSpreadX`/`_MaskOffset` remap **이전의 raw mesh UV**를 읽습니다 — 중심선이 x=0.5에 오려면 이게 필수입니다.

근거: `SmoothRegionMask.shader` (725–742, 1530–1546).

### 7.6 feather가 단순 blur slider가 아닌 이유

E3의 feather는 texture 좌표의 선형 blur 한 번이 아닙니다.

- texture texel 크기를 써서 해상도에 맞는 샘플 반경을 만듭니다.
- `_Feather`가 커지면 near/far tap 거리가 증가합니다.
- soft threshold와 core threshold를 따로 계산합니다.
- 립, eyeliner, lens, cheek, generated brow마다 기본 feather 상한이 다릅니다.

예를 들어 얇은 eyeliner에 cheek와 같은 feather를 쓰면 선이 그림자처럼 퍼집니다. 코드가 eyeliner 기본값을 더 작게 유지하는 이유입니다.
...
`lerp`, luma-preserving tint, soft clip, HSV/chroma 보정, matte/gloss 계산이 여기 속합니다. framebuffer blend **전에** source 색을 만드는 과정입니다.

### 8.2 GPU framebuffer blend

완성된 source를 이미 그려진 destination과 어떻게 합칠지 정합니다.

**ARwithFable 주요 blend state**

| Shader | Blend 방식 | 의미 |
|---|---|---|
| `FaceMakeup` | 명시적 Blend 없음, RGB/alpha 1 출력 | GrabPass 원본과 효과를 shader 내부에서 완성해 얼굴 영역을 교체 |
| `Lip` | `One, OneMinusSrcAlpha` | premultiplied alpha-over, 투명 gloss의 가산광 보존 |
...
| `LightingSim` | 최종 RGB/alpha 1 출력 | 메이크업까지 합성한 `_SceneGrab`을 전체 화면 grade 결과로 교체 |

**E3 주요 blend state** — [F2](#22-f2-screen-블렌드는-screen이-아니다) 참조.

### 8.3 straight alpha와 premultiplied alpha를 혼동하면

- Straight alpha 색에 `Blend One ...`을 쓰면 경계가 과하게 밝아질 수 있습니다.
- Premultiplied 색에 `Blend SrcAlpha ...`를 쓰면 alpha를 두 번 곱해 검은 테두리나 약한 색이 생깁니다.
- Multiply 경로에서 alpha 0만 반환해도 RGB가 검정이면 destination을 검게 만듭니다 → [7.5](#75-half-face에서-discard를-쓰는-이유).

현재 shader 주석과 blend state가 이 차이를 명시적으로 다룹니다.

---

## 9. Renderer가 mesh, material, 순서를 결합한다

### 9.1 ARwithFable renderer 그래프
...
**이미 씬에 있을 때만 찾아서 편입하는 것** (`FindAnyObjectByType`, null이면 no-op):

`StencilGuideRenderer` · `SplitMaskRenderer` · `LightingSimRenderer` · `TeethRenderer` · `DoubleLidRenderer` · `SymmetryGuideRenderer`

**즉 split/lighting/stencil renderer는 생성되지 않습니다.** 씬에 없으면 존재하지 않습니다. 편입된 객체는 `AuraStencilHost`의 `controlled` 목록에 들어가 함께 켜지고 꺼집니다 — [F4](#24-f4-일반-ar-화면에서-applyfilter는-아무도-받지-않는다) 참조.

`CanonicalFaceMesh`가 넓은 얼굴 surface를 그리고, 작은 정밀 부위는 각자의 landmark mesh를 따로 만듭니다. **하나의 UV atlas로 모든 문제를 풀려 하지 않고 부위별 최적 geometry를 쓰는 설계**입니다.
...
### 9.2 ARwithFable render queue

렌더 순서는 코드 호출 순서가 아니라 **material의 render queue로 고정**됩니다.
...
| Split mask(3100) < Lighting(3400) | **복원된 맨얼굴 반쪽도 같은 lighting grade를 받게** |
...
Renderer 설정: shadow cast off · receive shadow off · dynamic occlusion off · `sortingOrder = 120` · material `renderQueue = 5000` · shader `ZWrite Off`, `ZTest Always`, `Cull Off`.

따라서 E3 overlay는 일반 투명 object보다 **매우 늦게, depth test 없이** 그려집니다. 손·머리카락 가림은 depth buffer가 아니라 hand occlusion / skin gate / segmentation mask / tracking visibility 로직에 의존합니다.

**주의:** `SmoothRegionMaskMaterial.mat`의 직렬화된 queue는 **3000**이지만 런타임에 **5000**으로 덮어써집니다. Editor에서 asset만 보고 순서를 판단하면 틀립니다. (다만 3000은 shader의 `"Queue"="Transparent"` 태그 기본값이라 의도적 설정이라기보다 no-op 기본값입니다.)
...
## 10. 카메라 영상과 최종 프레임 합성

### 10.1 기본 AR Foundation 배경

Unity scene에는 `ARCameraBackground`, `ARCameraManager`, `ARFaceManager`, `XROrigin`이 있습니다. 기본 경로에서는 `ARCameraBackground`가 카메라 영상을 먼저 그리고 그 위에 face renderer들이 그려집니다.

근거: `MakeupARFaceValidation.unity` (366–403, 569–602).

### 10.2 시간 동기 `FramePresenter`

ARwithFable stencil graph가 활성화되면 `AuraStencilHost`는 `ARCameraBackground`를 끄고 `FramePresenter`를 켭니다. `FramePresenter`는 **landmark 추론에 사용한 바로 그 RGBA frame**을 `Texture2D`로 표시합니다.

장점은 배경과 landmark가 같은 시점의 frame이라는 점입니다. 최신 camera background와 약간 오래된 landmark를 섞을 때 생기는 **"화장이 얼굴에서 미끄러지는 느낌"** 을 줄입니다.

`FramePresenter`는 화면 비율·회전·mirror를 반영하고 `ImageToViewport()`를 renderer들과 공유하므로 배경과 makeup geometry가 같은 좌표 변환을 씁니다.
...
### 10.4 Unity frame을 React Native에 표시

React Native의 `UnityMakeupNativeView`가 iOS native component `AURAUnityMakeupView`를 요청합니다. Objective-C `UnityMakeupContainerView`가 singleton Unity root view를 자신의 subview로 옮기고 bounds에 맞춥니다.
...
화면이 사라지면 container는 Unity view를 분리하고 숨깁니다. **Unity player는 preload를 위해 살아 있을 수 있으므로, "renderer는 정상인데 RN 화면은 검정"인 문제는 shader가 아니라 native view mount 상태에서도 발생합니다.**

근거: `UnityMakeupNativeView.tsx` (12–50) · `UnityMakeupBridge.m` — `UnityMakeupContainerView` (685), `RCT_EXPORT_MODULE(AURAUnityMakeupView)` (769), view 생성 (778).

---

## 11. 한 픽셀이 만들어지는 전체 과정

풀페이스 E3 블러셔 픽셀을 예로 들면:

```mermaid
flowchart TD
    F[ARKit/AR Foundation face tracking]
    F --> M[ARFace vertices + triangles + UV]
    M --> DM[E3 dynamic region mesh]
    A[cheek mask texture] --> MAT[region material]
    P[color / opacity / intensity / feather] --> MAT
    DM --> VS[vertex shader]
    MAT --> FS[fragment shader]
    VS --> FS
    FS --> S1[UV로 mask sample]
    S1 --> S2[13-tap soft mask + threshold]
    S2 --> S3[cheek band / product color 계산]
    S3 --> B[multiply 또는 alpha blend]
    C[이미 그려진 camera pixel] --> B
    B --> OUT[최종 Unity pixel]
```
...
실제 shader는 부위별로 더 많은 보정이 있지만 **모든 단계는 이 다섯 값으로 이해할 수 있습니다.**

---

## 12. 부위별 구현 차이

| 부위 | 핵심 geometry/mask | 색 계산 | 주요 품질 보호 장치 |
|---|---|---|---|
| Foundation | canonical face mesh + 선택적 screen-space/segmentation 경로 | 원본 luma 유지하며 shade와 coverage 반영 | 눈/눈썹/입 제외, 턱선 기반 목 확장, chroma gate |
| Lip | 일반 경로는 landmark ring mesh, E3 생성 경로는 RGBA UV mask | luma-preserving pigment, base/tint/finish/gloss 스택 | 외곽 feather, 입 안쪽 ring 제외, corner/edge 보정, additive gloss |
| Blush | canonical UV/절차 R8 또는 E3 cheek mask | luma-preserving tint 또는 multiply filter | Gaussian tail, center/outer band, UV lift/spread |
| Brow | landmark band와 전용 renderer 또는 E3 다채널 mask | skin inpaint 후 brow pigment alpha-over | 자연 눈썹 neutralize, hair strand B 채널, mipmap/trilinear, profile fade |
| Eyeliner | landmark 기반 band/line 또는 얇은 E3 mask | alpha pigment + finish | 작은 feather, eye edge/wing geometry, 전용 queue |
| Lens | iris geometry 또는 runtime radial mask | 실제 iris를 유지한 color overlay/replace | pupil hole, limbal ring, blink visibility, stencil |

**핵심 통찰: 정밀한 경계는 texture보다 geometry가 낫습니다.** 립과 아이라인은 landmark mesh가 실제 경계를 만들고 shader는 그 안의 색과 마감을 담당합니다. 반면 블러셔·컨투어처럼 넓고 부드러운 영역은 UV 마스크가 더 효율적입니다.
...
| 자연 눈썹 제거 | 눈썹 위쪽 실시간 카메라 피부색과 brow mask | 기존 눈썹을 피부색으로 덮은 뒤 새 눈썹을 그림 |
| 목 영역 처리 | ARFace 턱선 아래 후보 mesh와 피부색 유사도 | 실제 목 피부만 파운데이션 영역으로 |

정확한 이해는 이렇습니다.

- 손은 **관절 검출 기반 occlusion mask**입니다.
- 눈썹 제거는 지우개가 아니라 **주변 피부를 복사해 덮는 inpainting**입니다.
- 목은 독립적 "목 객체 검출기"가 아니라 **턱선 기반 후보 영역 + 피부 판별 gate**입니다.
...
**하나라도 0이면 색 계산이 정상이어도 화면에 안 나타납니다.** 반대로 보호 mask가 1로 고정되면 손·머리카락·옷 위에도 메이크업이 그려집니다.
...
> ⚠️ **[F3](#23-f3-손-가림은-경로마다-메커니즘이-다르다) 참조:** 이건 E3 경로 전용입니다. ARwithFable 립/브로우는 세그멘테이션 기반 `OccludeGate`를 씁니다.

#### 처리 순서

1. E3에 립 또는 파운데이션 recipe가 활성화되면 hand runtime을 요청합니다.
2. `ARCameraManager.TryAcquireLatestCpuImage()`로 카메라 CPU frame을 가져옵니다.
...
**이 값을 소비하는 shader는 3개입니다** — `SmoothRegionMask.shader`(립·파운데이션), `ScreenSpaceFoundation.shader`, `FoundationSemanticComposite.shader`. 마지막 것은 `1-mask`가 아니라 occluder mask 자체를 `max` 합집합으로 씁니다.
...
| 활성 recipe | E3에 립 또는 파운데이션 recipe 필요 |
...
이 휴리스틱이 잘못된 후보를 고르면 **손은 검출됐어도 mask 위치가 틀어집니다.**
...
- foundation이나 E3 lip recipe가 없으면 `SetRuntimeRequested(false)`가 호출됩니다.
- CPU image 획득 실패, 낮은 confidence, ROI 미겹침, 500ms stale 중 하나만 발생해도 shader의 hand occlusion이 꺼집니다.
- **한 손만 요청합니다.** 양손이 동시에 보이면 다른 손은 보호되지 않습니다.
- 좌표 변환을 얼굴 겹침 점수로 추정하므로 mirror/rotation 상태 변화에 민감합니다.
- rect fallback은 있지만, 실제로는 **mask shader/RT 할당 실패 시에만** 발동합니다. pose 데이터가 부실해서 발동하는 게 아닙니다.

정적 판정: **E3 립·파운데이션용 손 가림 구현은 존재하고 정상 배선되어 있다. ARwithFable 경로는 다른(세그멘테이션) 가림 장치를 쓴다. 실제 실패 원인은 실기기 로그 없이는 `UNVERIFIED`.**
...
이 때문에 **multiply blend로는 구현할 수 없습니다.** 어두운 자연 눈썹을 밝은 피부색으로 덮어야 하므로, E3는 brow material을 강제로 `SrcAlpha / OneMinusSrcAlpha`로 바꿉니다.
...
1. `ARCameraManager.frameReceived`에서 Y/CbCr 또는 RGB 카메라 texture와 display transform을 받습니다.
2. **ARFace vertex 바운딩 박스의 정규화 위치**(왼볼 `0.30,0.52`, 오른볼 `0.70,0.52`, 턱 중심 `0.50,0.16`)를 화면에 투영해 피부 reference 색을 만듭니다. — 이름 붙은 landmark vertex가 아니라 박스 비율 지점입니다.
...
카메라 texture는 **0.5초 이내**의 최신 frame이어야 하고, 피부 reference anchor가 최소 하나 유효해야 합니다. 조건을 못 맞추면 `_SkinGateEnabled = 0`이 되며 — **셰이더의 자연 눈썹 inpaint는 정확히 no-op이 됩니다.** 새 눈썹만 그려지므로 기존 눈썹과 겹쳐 **"이중 눈썹"** 처럼 보입니다.

근거: `E3RegionMaskOverlay` (1097–1123, anchor 5916–5918, 신선도 5842–5848, anchor 판정 5851–5857, `ProjectSkinGateAnchor` 5924–5947) · `SmoothRegionMask.shader` — `SampleBrowInpaintSkin` (361–464), gate off 조기 반환 (365–368), 합성 (1150–1178, 1331–1349).

#### 정적 눈썹과 생성 눈썹의 차이

- 생성 눈썹은 **R 채널**을 제거 footprint로 씁니다.
- 정적/default 눈썹은 전용 R footprint가 없으므로 **새 눈썹의 coverage 자체를 제거 영역으로 재사용**합니다.
- 둘 다 이마 피부 inpaint 후 눈썹을 alpha-over로 그립니다.
- 앞머리가 눈썹을 덮은 것으로 판단되면 overlay alpha를 낮춰 실제 카메라 머리카락이 다시 보이게 합니다(`hairKeep`). **이건 셰이더가 카메라 픽셀을 직접 보고 판단하며, 모바일이 보낸 앞머리 신호와는 무관합니다** — [F1](#21-f1-눈썹-강도-컨트롤이-셰이더에-도달하지-않는다).
...
정적 판정: **E3 brow의 자연 눈썹 inpaint는 존재하며 live camera skin gate에 강하게 의존한다. 강도 control 두 개는 셰이더에서 소비되지 않고(F1), Vision eyebrow refinement도 자동 실행되지 않는다(F7).**
...
즉 geometry가 "어디를 검사할지"를 정하고, chroma gate가 "그 픽셀이 피부인지"를 판단합니다.
...
#### 현재 기본값: screen-space foundation

모바일 기본값은 foundation `mode='screenSpace'`, `fallbackMode='off'`입니다. 화면 위 카메라 pixel을 직접 보정하는 경로이며, **mesh foundation으로 자동 후퇴하지 않습니다.** 코드 주석이 의도를 명시합니다 — *"semantic mask가 준비되지 않으면 mesh painting으로 후퇴하지 말고 아무것도 보여주지 말 것."*
...
1. `FoundationMaskRuntime`이 ARFace mesh를 화면 좌표로 투영합니다.
2. 투영된 vertex의 convex hull에서 턱선 band를 찾습니다 (`NeckJawBandFraction = 0.30`).
3. 얼굴의 회전·기울기에 맞는 아래 방향을 계산합니다.
4. 턱선 아래로 좁아지는 neck extension mesh를 만듭니다 (`NeckBottomWidthFactor = 0.84`).
5. 이 mesh를 폭 **256px**(높이 192~512 clamp) screen-space mask에 그립니다.
6. shader가 neck 후보 pixel과 볼·턱 reference를 비교합니다.
7. 피부와 다른 머리카락·옷·배경을 mask에서 제거합니다.
...
근거: `FoundationMaskRuntime` — 투영 (909–995), hull (1197), band (1283–1291), `NeckJawBandFraction=0.30` (204), 아래 방향 (1399), 좁아짐 (1388–1390), `NeckBottomWidthFactor=0.84` (202), `MaskBaseWidth=256` (68), mask 텍스처 (685–715), 래스터화 (1104) · `FoundationMeshMask.shader` (116–124) · `ScreenSpaceFoundation.shader` (744–793, 크로마 수식 778–786).
...
여기에 더해 mesh mask 실패(예: `projected_triangle_count_low`)도 `NeckExtensionReady`를 false로 만듭니다(534).

**임계값은 3입니다** — legacy 경로의 `FoundationNeckMinColumns = 8`과 혼동하면 안 됩니다.

#### legacy `uvMask` fallback의 목 skirt

screen-space가 아니라 E3 mesh foundation을 쓸 때는 별도 3D neck strip을 만듭니다.
...
| gradient mask | 64×64 | (7043–7061) |

**두 가지 흔한 오해:**

- **"face 뒤로 0.02m 넣는다"** → 위쪽 행은 턱의 `z`를 그대로 유지하고 **아래 모서리만** 0.02m 뒤로 갑니다. 균일한 depth offset이 아니라 **갈퀴처럼 기울어진** 형태입니다.
- **"턱에서 진하고(=1.0) 아래로 갈수록 투명"** → 턱(v=0.98)에서 가중치는 **약 0.87**입니다. 얼굴 mesh와의 작은 겹침이 **턱선에 진한 줄무늬로 누적되는 것을 막으려는 의도**입니다. 가로 방향 edge fade도 있습니다.

추가로 column은 중앙 폭 `0.06 < normalizedX < 0.94`로 제한되고, 턱 곡선에 이웃 평균 smoothing이 2회 적용됩니다. 목에도 같은 live skin gate를 적용하지만 **설정은 다릅니다** (`_SkinGateCenterWeight`가 목은 1.0, 얼굴은 0.30).

근거: `E3RegionMaskOverlay` — 상수 (348–352), column 선택 (5449–5459), 최소 검사 (5486), smoothing (5493–5500), 길이 (5502), taper/offset (5513–5517), gradient (7043–7061), skin gate (5351, 5861).

**현재 기본 설정은 fallback이 `off`이므로 screen-space가 실패해도 이 legacy neck skirt로 자동 전환되지 않습니다.**

#### semantic segmentation 경로의 현재 상태
...
neck은 face와 **exclusion 세트가 다릅니다** — lip/eye/brow/priorExclusion을 무시합니다.
...
| camera texture 없음 | **quad가 아예 숨겨짐** (`SetQuadVisible(false)`) |
| display matrix·mirror·rotation 불일치 | 턱 아래 mask가 목과 다른 위치에 붙음 |
| cheek/chin 피부 reference 부정확 | 목 피부가 잘리거나 옷/배경이 남음 |
| semantic provider 비활성 | 머리카락·옷·배경 분리가 chroma 휴리스틱에만 의존 + opacity 0.55배 페널티 |
| `fallbackMode='off'` | screen-space 실패해도 legacy neck mesh가 안 나타남 |

> 셰이더의 `_CameraTextureMode < 0.5` 조기 반환(853–856)은 **방어용 죽은 분기**입니다. 카메라 texture가 없으면 그 전에 quad 자체가 숨겨지므로(563–571) 셰이더가 실행되지 않습니다. 진짜 원인을 여기서 찾으면 안 됩니다.

정적 판정: **목 확장 코드는 존재하지만 현재 기본 경로는 완전한 목 segmentation이 아니다. 진짜 semantic skin class는 저장소에 아예 없으므로(F7), geometry와 피부색 휴리스틱의 정확도가 결과를 좌우한다. 실기기에서 어느 gate가 실패하는지는 runtime log/GPU mask 확인 전까지 `UNVERIFIED`.**
...
3. 눈썹은 생성 마스크의 R/G/B 채널과 `_SkinGateEnabled`를 각각 확인합니다. (강도 uniform은 봐야 소용없습니다 — F1)
4. 목은 foundation 진단의 `neckExtensionReady`, mask orientation, camera texture mode를 확인합니다.
5. `FoundationSegmentationRegistry.ActiveProviderName`이 실제 provider를 가리키는지 확인합니다.
6. debug mask 화면에서 geometry prior와 최종 skin-gated mask를 따로 비교합니다.

세 기능 모두 최종 색상보다 **renderer 경로 → 입력 frame → 좌표 변환 → 보호 mask → shader uniform** 순서로 확인해야 원인이 빨리 분리됩니다.
...
- Softness를 bucket 캐시해 slider 변화마다 texture를 재생성하지 않습니다.
- E3 static mask를 캐시하고 wrap/filter를 한 번만 설정합니다.
- Generated brow는 mipmap/trilinear로 원거리 shimmer를 줄입니다.
- `SmoothRegionMask`에서 예전 per-region GrabPass를 제거해 full-screen copy 중복과 flicker 위험을 줄였습니다.
- ARwithFable shader들이 같은 이름 `_CameraFeed`를 공유해 Built-in pipeline의 named GrabPass 재사용을 의도합니다.
- Renderer queue를 중앙 상수로 관리해 queue 충돌을 줄였습니다.

### 14.2 비용이 큰 지점

- 512×512 RGBA 마스크를 Base64 JSON으로 보내면 payload와 임시 복사 비용이 큽니다 (raw 1.0 MiB → Base64 1.33 MiB → 여러 단계 복사).
- `SmoothRegionMask`의 기본 soft mask는 fragment당 마스크 texture를 **13회** 샘플하고, region 분기에 따라 추가 sample을 수행합니다.
- Lip gloss가 활성화되면 동일 mesh가 additive 두 번째 pass를 한 번 더 그립니다.
- E3는 얼굴×활성 부위별 dynamic mesh와 material instance를 유지합니다.
- `ZTest Always`와 높은 queue는 early depth rejection 이점을 거의 못 씁니다.
- Screen-space segmentation/foundation은 전체 화면 fragment cost를 추가합니다.
...
- 마스크 1개와 5개 활성화 시 GPU frame time 차이
- gloss, segmentation, lighting grade 각각의 추가 비용
...
| 증상 | 우선 의심할 원인 |
|---|---|
| 화장이 얼굴 전체에 칠해짐 | shader가 R을 읽는데 authored shape가 alpha에만 있음, sRGB/채널 계약 오류 |
| 위치가 눈·입에서 어긋남 | 다른 topology용 UV 마스크, V flip, mirror/rotation 불일치 |
| 경계가 검거나 탁함 | straight/premultiplied alpha와 Blend state 불일치 |
| 고개를 돌리면 길게 늘어남 | canonical UV와 실제 landmark 경계 차이, profile triangle/normal 처리 부족 |
| 떨림 | face vertices tracking noise, 마스크 mipmap 부족, 배경과 landmark frame 시점 불일치 |
| 손이 얼굴 앞을 지나도 화장이 위에 보임 | 경로 확인 먼저(F3) → E3면 hand/segmentation occlusion 비활성 또는 stale |
...
| recipe는 전송됐지만 아무것도 안 보임 | Unity runtime 준비 전 메시지, renderer disabled, face tracking 없음, mask texture 누락 |
| Unity에서는 그리는데 RN 화면이 검정 | Unity root view가 container에서 분리됨, native view mount/visibility 문제 |

### 15.2 권장 진단 순서

1. RN이 **올바른 renderer 경로**에 메시지를 보냈는지 확인합니다. (F3·F4 때문에 이게 1번입니다)
2. Unity bridge가 method와 payload를 실제 invoke했는지 확인합니다.
3. face tracking과 UV 수가 유효한지 확인합니다.
4. mask texture가 등록/로드되었고 예상 채널에 값이 있는지 확인합니다.
5. shader debug mask mode로 raw/processed mask를 직접 봅니다.
6. material의 color, opacity, threshold, feather, blend factor를 확인합니다.
7. renderer enabled, mesh triangle 수, render queue를 확인합니다.
8. hand/skin/segmentation gate가 결과를 0으로 만드는지 확인합니다.
9. 마지막으로 native Unity view가 RN container에 mount되었는지 확인합니다.

이 순서는 "색이 이상하다"는 문제를 곧바로 shader 수정으로 몰고 가지 않고 **데이터 → geometry → material → renderer → view** 계층을 차례로 분리합니다.
...
- UV 마스크, product parameter, shader, renderer 책임이 코드상 분리되어 있습니다.
- 정밀 경계는 landmark geometry, 넓은 soft region은 UV texture로 처리하는 hybrid 선택이 합리적입니다.
- 색을 평평하게 덮지 않고 실제 camera luma를 보존하는 경로가 다수 존재합니다.
- 마스크를 linear/uncompressed data로 import하여 threshold 계약을 지킵니다.
- blend mode, queue, occlusion, texture cache가 명시적입니다.
- 생성 마스크는 schema·local-only·byte count를 검증합니다 (다만 립은 F5).
- tracking noise, mip shimmer, frame synchronization 등 실기기 AR 문제를 코드에서 직접 다룹니다.
- 셰이더 주석이 **왜 그렇게 했는지**(discard 이유, 턱 줄무늬 방지 등)를 남겨 놓았습니다.

### 16.2 복잡성과 위험

- 두 render graph와 hybrid lip routing 때문에 **한 UI 조작이 어느 renderer에 도달하는지 추적 비용이 큽니다.**
- 이름이 같은 `texture`, `mask`, `rendererMode`가 경로마다 다른 의미를 가집니다.
- E3 `SmoothRegionMask.shader` 하나가 foundation, lip, cheek, brow, eyeliner, lens를 모두 분기 처리하여 shader variant가 아니라 **runtime branch 복잡도**가 높습니다.
- queue 5000 + `ZTest Always`는 결과를 강제로 위에 보이게 하지만 실제 3D occlusion 책임을 별도 mask에 넘깁니다.
- Base64 texture transport는 구현은 단순하지만 메모리·지연 면에서 확장성이 낮습니다.
- **끊어진 계약이 여럿 있습니다** (F1·F5·F8). 코드는 값을 운반하지만 소비자가 없습니다.
- **주석이 코드보다 먼저 썩었습니다** (F8). 낡은 주석은 없는 주석보다 위험합니다.

### 16.3 권장 방향

단기적으로는 새 shader를 추가하기보다 **renderer ownership 표준화와 끊어진 계약 정리**가 우선입니다.

1. 화면 모드별 활성 renderer를 하나의 route-level controller가 명시적으로 켜고 끕니다.
...
5. **F2 결정:** `screen` 이름과 실제 blend state가 다르면 contract를 수정하거나 진짜 구현을 추가합니다.
6. **F5 수정:** 립도 브로우처럼 등록 실패를 거부해야 합니다.
7. **F8 정리:** 죽은 코드(`UpdateHandOcclusion`), 죽은 채널(mask A), 낡은 주석(4521)을 제거합니다.
8. Generated texture transport는 장기적으로 파일/공유 buffer/네이티브 texture 경로를 검토합니다.
9. GPU capture에서 pass 수, GrabPass, overdraw, texture bandwidth를 측정한 뒤 shader 분리를 결정합니다.
...
| RN bridge | `apps/mobile/src/features/ar/services/unityMakeupBridge.ts` | FilterParams, recipe, generated mask 직렬화·전송 |
| RN recipe contract | `apps/mobile/src/shared/contracts/fullFaceMakeupRecipe.ts` | region layer, maskTextureId, blend/finish 파라미터 정의 |
| RN native view | `apps/mobile/src/features/ar/components/UnityMakeupNativeView.tsx` | iOS native component 요청 |
| iOS bridge/view | `apps/mobile/ios/AURA/UnityMakeupBridge.m` | UnityFramework 수명주기, UnitySendMessage, native view mount |
...
| ARwithFable controller | `apps/unity/MakeupAR/Assets/Scripts/MediaPipeGraft/ARwithFable/Face/MakeupController.cs` | FilterParams를 material/renderer에 분배 |
| 얼굴 landmark | `.../ARwithFable/Face/FaceLandmarkSource.cs` | MediaPipe 478 landmark와 frame 공급 |
| canonical mesh | `.../ARwithFable/Face/CanonicalFaceMesh.cs` | 468 face mesh topology, UV, 매 프레임 vertex 갱신 |
| mask 생성 | `.../ARwithFable/Face/MaskGenerator.cs` | Resources mask 조회, 256 R8 절차 mask와 cache |
| 기본 shader | `apps/unity/MakeupAR/Assets/Resources/FaceMakeup.shader` | 카메라 기반 피부/베이스/블러셔/컨투어 합성 |
| 립 renderer/shader | `.../ARwithFable/Face/LipRenderer.cs`, `Assets/Resources/Lip.shader` | landmark ring mesh와 premultiplied lip 합성 |
| **Fable 가림 게이트** | `apps/unity/MakeupAR/Assets/Resources/Occlusion.cginc` | **세그멘테이션 기반 `OccludeGate` (F3)** |
| render order | `.../ARwithFable/Face/MakeupQueues.cs` | 부위별 queue 계약 |
| camera presenter | `.../ARwithFable/Face/FramePresenter.cs` | landmark와 같은 frame 표시, mirror/rotation/aspect 매핑 |
...
| full-face Unity bridge | `apps/unity/MakeupAR/Assets/Scripts/RNBridge.cs` | recipe/generated mask parse와 E3 dispatch |
| E3 overlay | `apps/unity/MakeupAR/Assets/Scripts/E3RegionMaskOverlay.cs` | Texture2D 등록, ARFace UV mesh, material, visibility |
| E3 shader | `apps/unity/MakeupAR/Assets/Shaders/SmoothRegionMask.shader` | 다채널 mask, feather, region branch, alpha/multiply/gloss |
| 손 관절 runtime | `apps/unity/MakeupAR/Assets/Scripts/E7HandOcclusionRuntime.cs` | AR camera CPU frame, Vision hand pose, ROI 판정, 손 mask 수명주기 |
| 손 mask shader | `apps/unity/MakeupAR/Assets/Resources/HandOcclusionMask.shader` | 21개 관절을 circle/capsule로 연결한 screen-space mask |
...
| generated brow 생성 | `apps/mobile/src/features/ar/services/browGenerateCore.ts` | R neutralize, G body, B strand RGBA mask와 payload 생성 |
| screen-space foundation | `apps/unity/MakeupAR/Assets/Scripts/ScreenSpaceFoundationController.cs` | foundation mode, projected mask, hand/segmentation uniform |
| foundation mask runtime | `apps/unity/MakeupAR/Assets/Scripts/FoundationMaskRuntime.cs` | ARFace 투영, feature exclusion, 턱선 기반 neck extension mask |
| foundation shader | `apps/unity/MakeupAR/Assets/Resources/ScreenSpaceFoundation.shader` | 피부색 gate, neck gate, 손·hair·brow exclusion과 색 보정 |
| semantic compositor | `apps/unity/MakeupAR/Assets/Scripts/FoundationSemanticMaskCompositor.cs` | geometry prior와 skin/hair/feature/hand mask 합성 |
| semantic shader | `apps/unity/MakeupAR/Assets/Resources/FoundationSemanticComposite.shader` | face/neck 분리 합성 후 MAX 결합, 손 가림 3번째 소비처 |
| Vision parsing provider | `apps/unity/MakeupAR/Assets/Scripts/VisionFaceParsingProvider.cs` | face parsing mask 공급 경로와 현재 capture guard |
| bridge parsing provider | `apps/unity/MakeupAR/Assets/Scripts/BridgeFaceParsingProvider.cs` | 외부 R8 class mask를 segmentation registry에 등록 |
| mask importer | `apps/unity/MakeupAR/Assets/Editor/SmoothRegionMaskTextureImporter.cs` | linear/readable/uncompressed mask import 계약 |
| 정적 mask 자산 | `apps/unity/MakeupAR/Assets/Resources/SmoothRegionMasks/` | E3 정적 마스크 PNG 46장 |
| E3 material template | `apps/unity/MakeupAR/Assets/Resources/SmoothRegionMaskMaterial.mat` | 직렬화 queue 3000 (런타임 5000으로 덮어씀) |
...
| UV/mask/texture/material/shader/renderer 정적 연결 | CONFIRMED |
...
| E3 손 관절 검출 → hand mask → shader visibility 경로 | CONFIRMED |
| ARwithFable 립/브로우의 세그멘테이션 `OccludeGate` (F3) | CONFIRMED |
| E3 자연 눈썹 skin inpaint 경로 | CONFIRMED |
| skin gate off 시 inpaint가 정확히 no-op (F1 연관) | CONFIRMED |
| brow cleanup/neutralize strength control 미소비 (F1) | 정적 불일치 CONFIRMED |
...
| screen-space 턱선 → neck candidate → chroma gate 경로 | CONFIRMED |
| `ScreenSpaceFoundationController.UpdateHandOcclusion` 미호출 (F8) | CONFIRMED |
| 생성 브로우 mask A 채널 미사용 (F8) | CONFIRMED |
...
| ARwithFable 8개 shader의 blend state 전부 | CONFIRMED |
...
| 위치 | 수정 전 | 수정 후 |
|---|---|---|
| 목 segmentation | "native parsing 코드는 compile define까지 필요하다" | **define은 이미 켜져 있음.** 죽은 이유는 `#if` 안쪽의 flicker guard |
| 목 chroma gate | "색상(hue)·밝기를 비교" | **hue 아님** — r/b 크로마 비율 + 휘도 비율 |
...
| 생성 브로우 A 채널 | "새 눈썹 body alpha" | 생성기만 기록, **셰이더는 읽지 않음** |
| 손 좌표 8모드 선택 | "겹침이 가장 많은 변환" | 겹침 + 중심 보너스 0.14 + 점 보너스 ≤0.18 |
| 손 mask 소비 셰이더 | 2개 | **3개** (`FoundationSemanticComposite.shader` 누락) |
| 손 mask RenderTexture | "256px" | 폭만 256 고정, **높이는 유도 후 128~768 clamp** |
| `_CameraTextureMode=0` | "shader가 투명 결과 반환" | 방어용 **죽은 분기** — 그 전에 quad가 숨겨짐 |
| ARwithFable renderer 목록 | "split/lighting/stencil renderer를 **만든다**" | **만들지 않음** — 씬에 이미 있을 때만 `FindAnyObjectByType`으로 편입 |
| ARwithFable renderer 목록 | `LipStyleRenderer` 누락 | 생성 목록에 추가 |
...
| ARwithFable 활성화 (F4) | "`applyFilter` subscriber와 renderer가 꺼져 있을 **가능성이 있다**" | **확정적으로 버려짐.** `MakeupController`가 `OnEnable`에서 구독하므로 비활성화 = 구독 해제. 문서가 과소 서술했음 |
...
- **2장 "지금 알아야 할 것" 신설** — §7.2, §12.2, §12.3, §15에 흩어져 있던 발견을 한곳에 모음
...
1. **립 마스크 등록 실패를 브로우처럼 거부할 것인가?** (`RNBridge` 858)
