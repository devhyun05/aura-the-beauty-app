...
- **0장. 큰 그림** — AR 메이크업은 결국 뭘 하나 · E3와 Fable의 정체(두 개의 독립 엔진) · E3는 fallback인가 · 왜 두 개인가 · 같은 지도를 나눠 쓰는 게 아니다
- **1장. 얼굴 찾기** — 미리 vs 실시간 · MediaPipe vs ARKit · 점이 많으면 더 정밀한가 · 립은 누가 그리나
- **2장. 마스크** — 왜 흑백인가 · 게임처럼 색을 구우면 안 되나 · 미리 그린 건가 내 얼굴로 만든 건가 · 슬라이더는 뭘 바꾸나 · 채널 약속 · ⚠곱셈 구조
- **3장. 텍스처** — 마스크와 뭐가 다른가 · AI 생성 마스크의 정체(LLM 아님) · 실사용과 효과 · 마스크의 여행(Base64) · 왜 눈썹만 밉맵 · ⚠립만 조용히 통과 · 색 이미지 취급 금지
- **4장. 셰이더** — 왜 그냥 칠하면 안 되나 · 반투명은 왜 안 되나 · 색도 비치는 게 자연스럽지 않나 · 경계가 안 보이는 원리 · 페더는 블러인가 · 반쪽 비교의 함정
...
- **9장. 립** — 립을 그리는 세 방식 · 링 메시가 푸는 두 문제 · 페더는 바깥만 · 광택은 빛을 더한다 · 맞춤 마스크의 한계 · 층 순서
...
**Q. E3와 Fable은 정확히 뭔가요? "렌더링 방식"이라기엔 큰 것 같은데요.**

A. 맞습니다 — 그리기 기법 하나가 아니라, **한 앱 안에 들어 있는 두 개의 독립된 화장 엔진**입니다. 각자가 얼굴을 찾는 눈부터 손 가림까지 전 층을 따로 갖고 있습니다.

| 층 | Fable | E3 |
|---|---|---|
| 받는 주문서 | 포인트 메이크업 요청 | 풀페이스 레시피 / 맞춤 생성 마스크 |
| 얼굴 찾는 눈 | MediaPipe | ARKit |
| 배경 표시 | 자체 (시간 동기 프레임) | AR Foundation 기본 |
| 화장 도안 | 링 메시 + 절차 마스크 | 지도 위 마스크 (공용/맞춤) |
| 셰이더 | 부위별 전용 여러 개 | 만능 셰이더 하나가 분기 |
| 합성 철학 | 재구성 (바꿔치기) | 오버레이 (스티커) |
| 손 가림 | 세그멘테이션 게이트 | Vision 손 관절 |

한 주방에 요리사가 두 명인데 각자 자기 냉장고·칼·레시피·화구를 따로 쓰는 상황입니다 — 같은 손님상(화면)에 올릴 뿐, 전 과정이 별개입니다. 공유하는 건 카메라와 최종 화면뿐이에요. 이 문서에서 "경로"라고 부르는 것의 실체가 이 엔진들입니다.

이름 자체는 기술 용어가 아니라 **팀 내부 코드명**입니다(Fable = MediaPipe 시스템 이식 작업명, E3 = 지도-마스크 오버레이 작업명). 이름에서 정보가 안 나오는 게 정상입니다.

이 프레임이 중요한 이유: "엔진이 다르다"는 건 **같은 증상이라도 원인 후보 목록 자체가 다르다**는 뜻입니다. 미끄러짐이 E3에서만 나오는 것(시간 동기 유무), 손 가림 로그가 서로 다른 것이 전부 여기서 나옵니다. 진단 1번이 항상 "지금 어느 엔진인가"인 이유입니다.

**Q. 그럼 E3는 Fable의 fallback(예비)인가요?**

A. 아니요 — "주전 대 후보"가 아니라 **담당 구역이 나뉜 두 팀**입니다. 진입점마다 관계가 다릅니다.

| 진입점 | 주전 | 관계 |
|---|---|---|
| 일반 AR 화면 | **사실상 E3** | Fable이 주전이어야 하지만 수신자가 꺼져 있어(부록 B) E3가 실질적으로 그리는 중 |
| 풀페이스 레시피의 립 | Fable | **여기만 진짜 fallback 관계** — Fable 립 렌더러가 없을 때만 E3 립이 그림 |
| 풀페이스 나머지 부위·맞춤 생성 마스크 | E3 | fallback이 아니라 **유일한 담당** — Fable엔 이 주문서를 받는 입구가 없음 |
| 튜토리얼(스텐실) 화면 | Fable | 유일한 담당 |

주의할 것: "fallback"이라는 말이 실제로 나오는 곳은 **엔진 사이가 아니라 E3 안**입니다 — 파운데이션의 화면 공간 방식이 실패하면 옛 메시 방식으로 후퇴할지 설정(현재 꺼짐, 12장). 엔진 사이 층위와 엔진 안 층위를 섞으면 헷갈립니다.
...
| **E3** | 얼굴 지도 위에 스텐실을 대고 칠하기 | 파운데이션·블러셔처럼 **면이 중요한** 화장, 맞춤 생성 마스크 |

지금은 전환기라서 경계가 깔끔하지 않습니다 — 립은 E3로 요청해도 Fable이 대신 그리는 우회가 진행 중입니다. "내가 만진 화장이 지금 어느 경로로 그려지고 있나"를 먼저 확인하는 습관이 모든 진단의 1번입니다.

**Q. 두 경로라는 게, 같은 얼굴 지도(UV)의 다른 부분을 쓰는 정도 아닌가요?**

A. 흔한 오해인데, 아닙니다 — **지도 자체가 다릅니다.** 차이는 세 겹입니다.

① **얼굴 그물과 지도가 다릅니다.** Fable은 MediaPipe의 468점 그물과 그 전용 지도를, E3는 ARKit의 약 1,220점 그물과 Apple의 지도를 씁니다. 점 배치도 지도 펼침도 달라서, **한쪽 지도용으로 그린 마스크는 다른 쪽에 못 씁니다** — 마스크가 두 경로 사이에 공유되지 않습니다.

② **경계를 만드는 방법이 다릅니다.** E3는 모든 부위를 "지도 위 마스크"로 처리하지만, Fable은 정밀 부위(립·아이라인·눈썹)에서 마스크를 아예 안 쓰고 랜드마크로 전용 지오메트리를 만듭니다. Fable의 립에는 "지도의 립 부분"이라는 것 자체가 없습니다.

③ **합성 철학이 다릅니다 — 가장 근본적인 차이입니다.**

```text
Fable = 재구성:  화면을 먼저 찍음 → 그 픽셀을 재료로 화장까지 완성한
                새 픽셀을 만들어 → 얼굴 영역을 통째로 교체
E3    = 오버레이: 카메라 위에 반투명 스텐실 판을 겹쳐 얹음
                (무조건 최상단, 깊이 무시)
```

화가에 비유하면 Fable은 사진을 받아 **다시 그려서 바꿔치기**하고, E3는 사진 위에 **반투명 스티커를 붙입니다.** 파생 차이가 전부 여기서 설명됩니다 — Fable은 경계 이음새가 원리적으로 없고(교체니까), E3는 손 가림 장치가 따로 필요합니다(스티커는 깊이를 모르니까). "같은 지도의 다른 부분"이 맞는 말이 되는 건 E3 안에서 립 마스크와 볼 마스크를 비교할 때뿐입니다.
...
**Q. 게임에서는 텍스처에 색까지 굽고 셰이더는 조명만 계산하잖아요. 왜 여기선 안 그러나요?**

A. 게임 텍스처가 들고 있는 두 가지가, 여기서는 **둘 다 런타임에 외부에서 오기** 때문입니다.

```text
게임:   최종색 = 텍스처의 색 × 계산된 조명
이 앱:  최종색 = 상품 DB의 색 × 카메라가 찍은 실제 피부 밝기
                 ↑ 런타임에 바뀜    ↑ 런타임에 바뀜
```

- **색이 데이터입니다.** 립스틱 색은 상품마다 다르고 상품은 수천 개입니다. 색을 텍스처에 구우면 색상별 텍스처가 필요하지만, 무채색 도안 1장 × 아무 색 = 조합으로 끝납니다.
- **구워 봤자 못 씁니다.** 최종색은 "당신 피부 밝기 × 제품 색"으로 계산됩니다(4장). 텍스처를 만드는 시점엔 당신의 조명·피부를 알 수 없으니, 색을 구워도 셰이더가 도로 분해해야 합니다.
- **빈 채널이 남는 게 이득입니다.** 색을 안 넣으니 세 칸이 남고, 거기에 서로 다른 지시를 넣습니다(아래 채널 약속). 색 텍스처였다면 그 칸들을 전부 "보이는 모습"에 써 버렸겠죠.

한 줄 요약: **모양은 텍스처가, 색은 상품 데이터가, 밝기는 카메라가** 담당하는 3자 분업입니다.
...
| **맞춤 생성** | 당신 얼굴의 랜드마크로 폰에서 그려 전송 (3장) | **립·눈썹만** (당신 전용) |
...
**Q. "AI 생성 마스크"가 정확히 뭔가요? 폰에서 LLM에 요청하는 건가요?**

A. LLM도, 서버도 아닙니다 — **전 과정이 폰 안에서** 끝납니다.

```text
① 카메라 프레임 한 장
② Apple Vision/MediaPipe가 얼굴 랜드마크 분석    ← "AI"는 여기까지
③ 코드가 그 랜드마크(눈썹·눈·관자놀이·윤곽)로 영역을 기하학적으로 계산
④ 512×512 이미지 픽셀을 직접 채움 (R=지울 곳, G=새 눈썹, B=털 결)
⑤ Unity로 전송
```

AI가 하는 일은 랜드마크를 찾는 것까지이고, 마스크 자체는 그 점들로 도형을 그리는 **규칙 기반 코드**입니다. 서버로는 못 나가게 계약으로 막혀 있습니다 — 전송 시 "기기 내 처리·서버 업로드 없음·원본 프레임 장기 저장 없음"을 명시해야 하고 Unity가 검증해서 어기면 거부합니다.

**Q. 그럼 왜 "AI 생성"이라고 부르나요?**

A. 부르는 쪽이 부정확한 겁니다. 코드의 실제 이름은 전부 그냥 **"생성(generated)"** 이고, "AI"는 문서와 대화에서 붙은 수식어입니다. 근거가 아예 없진 않습니다 — 랜드마크 검출은 진짜 신경망이니까요. 하지만 "AI가 개입한다"(검출)와 "AI가 생성한다"(래스터화)는 다른 말입니다.

이 이름이 위험한 이유: "AI가 생성한다"고 믿으면 *"품질이 아쉬우면 더 좋은 생성 모델로 바꾸자"* 는 가설이 나오는데, **바꿀 생성 모델이 존재하지 않습니다.** 마스크 모양의 진짜 손잡이는 두 개뿐입니다 — ① 랜드마크 정확도(여기만 모델 교체가 유효), ② 점에서 윤곽을 만드는 도형 계산 규칙(코드 수정). 이름 하나가 존재하지 않는 손잡이를 만들어내는 사례입니다. 이 문서에서는 관례를 따라 "생성 마스크"로 부르되, AI라는 말은 검출 단계에만 씁니다.

**Q. 실제로 쓰이고 있나요? 효과는요?**

A. 배선은 화면까지 완전합니다 — 개인화 생성 서비스, AR 필터 화면의 생성 눈썹 훅, 캡처 화면 세 곳이 발신하고, Unity 검증을 거쳐 E3가 렌더합니다. 주 용도는 **눈썹**이고 립은 "검증 경로"라는 이름대로 실험적 위치입니다.

효과는 갈립니다. **눈썹은 원리적 우위가 명확** — 정적 눈썹은 "지울 곳" 정보가 없어 꼼수(10장)를 쓰지만, 생성 마스크는 당신의 실제 자연 눈썹 위치를 전용 채널로 담아 옵니다. 단 F1(강도 계약 끊김)과 이중 눈썹 조건이 효과를 깎습니다. **립은 한계가 실증적** — 모양만 풀고 배치를 못 풀어(9장) 링 메시로 대체 중입니다. 실기기 화면 품질 자체는 미측정입니다.

**Q. 맞춤 생성된 내 입술 마스크는 어떻게 Unity까지 오나요?**
...
**Q. ⚠ 잘못된 립 마스크가 조용히 통과한다던데, 아직도 그런가요?**

A. **2026-07-17에 수리됐습니다.** 먼저 "잘못된"의 뜻부터 — 맞춤 마스크는 "가로 512 × 세로 512"라는 신고서와 실제 픽셀 데이터가 함께 오는데, Unity가 **신고한 크기 × 4칸(RGBA)만큼 데이터가 정확히 왔는지** 검산합니다. 이 검산에 실패한 것(해상도 불일치, 전송 중 잘림 등)이 잘못된 마스크입니다.

예전에는 **눈썹은 거부하는데 립은 경고만 남기고 계속 진행**하는 비대칭이 있었습니다(텍스처 없이/옛 마스크로 그려짐). 지금은 **립도 눈썹처럼 거부**하고, RN에는 재시도하지 말라는 **터미널 사유**를 돌려줍니다. 게다가 RN측이 `generatedMaskId`+revision으로 이벤트를 상관 지어, 오래된 마스크 결과가 새 요청을 덮어쓰지 못하게 막습니다. "맞춤 립이 이상하다"면 여전히 **등록이 실제로 됐는지(경고/거부 로그)** 부터 확인하는 게 제일 싼 검사입니다.

**이게 없으면? — 마스크를 수치 데이터로 취급하지 않는다면.** 이미지 파일은 사람 눈 기준으로 숫자를 구부려 저장합니다(감마 보정). 마스크를 "사진"으로 취급하면 읽는 순간 되돌리기 변환이 몰래 끼어드는데, 이 변환은 **양끝(0%·100%)은 그대로 두고 중간 농도만 왜곡**합니다 — 50%가 22%쯤으로 읽히는 식입니다. 문제는 마스크의 경계가 바로 그 중간 농도 경사면이라는 것: "농도가 문턱을 넘는 곳까지 바른다"에서 문턱을 넘는 지점이 이동해, **도안도 코드도 안 건드렸는데 경계 위치가 밀립니다.** 그래서 마스크는 임포트할 때부터 색 보정 없음·무압축으로 강제합니다.
...
**Q. 밝기 말고 원본 색도 좀 비치는 게 더 자연스럽지 않나요?**

A. 맞습니다 — 그리고 실제로 비칩니다. "색 완전 교체"는 강도 1.0일 때 이야기이고, 마지막 단계가 "원본과 물들인 색을 **강도만큼 섞기**"라서 강도 0.6이면 원본 색도 40% 비칩니다. E3에는 원본 색이 항상 참여하는 **곱셈 모드**도 있습니다(블러셔 계열).

그런데 기본 모델이 색 교체인 건 자연스러움이 아니라 **제품 색 충실도** 때문입니다. 트라이온의 존재 이유는 "이 색이 나한테 어울리나"인데, 원본 입술색이 항상 섞이면 레드가 탁한 주황으로, 누드가 안 나오는 색으로 보입니다 — 사용자가 보는 게 제품 색이 아니게 되죠. 진한 매트 립은 현실에서도 입술색을 덮으니 색 교체가 물리에도 맞습니다.

정답은 제품 마감에 따라 다릅니다: 매트 풀커버 = 색 교체 + 높은 강도, 틴트·셔 = 낮은 강도 또는 곱셈. **"틴트 계열이 페인트같이 보인다"면 색 통과량(강도·블렌드 모드)을 늘리는 게 유효한 개선 가설**이고, 손잡이는 전부 이미 존재해서 수리 없이 실험 가능합니다.

**Q. 파운데이션은 어떻게 경계가 안 보이나요?**

A. Fable의 얼굴 셰이더는 "스티커 얹기"가 아니라 **"복사해서 고쳐 쓰고 바꿔치기"** 방식입니다.

```text
① 찰칵 — 그리기 직전의 화면을 통째로 복사해 둔다
② 얼굴 영역의 픽셀마다: 복사본에서 원본 색을 읽고, 그 색을 재료로
   "화장한 버전"을 계산한다 (효과가 0인 자리는 계산 결과 = 원본 그대로)
③ 계산한 색으로 그 자리를 완전히 덮어쓴다 (겹침이 아니라 교체)
```

시험지를 복사해서 몇 문제만 고쳐 쓴 뒤 원본과 통째로 바꿔치기하는 것과 같습니다 — 안 고친 문제는 글자가 원본과 완전히 똑같으니 바꿔치기한 티가 안 납니다.

경계가 안 보이는 이유가 여기서 나옵니다. 얼굴 영역의 가장자리는 화장 효과가 0인 곳이라 **"원본을 원본으로 덮어쓰는"** 일이 일어납니다 — 결과 픽셀이 원래와 완전히 같으니, 경계선이 흐릿한 게 아니라 **경계라는 것이 결과물에 존재하지 않습니다.** 반면 스티커 방식(E3)은 아무리 옅어도 "판의 끝"이 존재해서, 조금만 어긋나면 윤곽이 보입니다.

공짜는 아닙니다 — 매 프레임 화면 복사 비용이 있어서, 넓은 면적에 이음새가 치명적인 얼굴 바탕(파운데이션·블러셔 계열)에만 씁니다.
...
A. 먼저 배경 하나 — GPU는 화장을 한 번에 다 그리지 않고 **순번대로 한 부위씩 캔버스에 칠합니다**(6장). 블러셔 차례가 왔을 때 캔버스에는 이미 "카메라 + 파운데이션"이 칠해져 있죠. 이것이 "이미 그려진 화면"입니다 — **내 차례에 캔버스에 먼저 칠해져 있는 모든 것.**

이제 블러셔 픽셀 하나로 두 층을 보면:

```text
1층 (셰이더 계산): "이 자리의 블러셔는 이 분홍, 진하기 30%"
                   → 팔레트에서 물감을 조색하는 단계
2층 (GPU 합성):    그 분홍을 캔버스색과 무슨 산수로 섞나?
                   일반 규칙: 결과 = 분홍×0.3 + 캔버스색×0.7
                   → 밑그림 위에 붓을 대는 방식을 정하는 단계
```

증상으로 범인을 가를 수 있는 이유: 2층의 "섞기"가 많이 일하는 곳은 **두 색이 실제로 섞이는 자리**뿐입니다 — 진하기가 100%→0%로 줄어드는 **가장자리**, 그리고 파운데이션 위의 블러셔처럼 **화장이 겹치는 자리**. 화장 한가운데(진하기 100% 근처)는 섞기가 거의 없어 1층의 색이 그대로 보입니다.

```text
중앙부까지 색 자체가 이상   → 물감이 틀림     → 1층 (셰이더 계산)
중앙은 멀쩡, 가장자리 테두리
·겹친 자리만 이상          → 붓질이 틀림     → 2층 (합성 규칙)
```

**Q. ⚠ screen 블렌드가 가짜라면서요?**

A. 예전엔 그랬고, **2026-07-17에 계약에서 아예 제거**됐습니다. 옛날엔 합성 방식으로 `screen`(밝게 겹침)을 요청할 수 있었지만 실제 GPU 설정은 `normal`(일반 겹침)과 **완전히 동일**해서, 이름만 보고 "밝게 합성되겠지"라고 세운 가설은 검증 전에 이미 틀린 것이었습니다. 지금은 계약(union)에 `screen` 자체가 없고, 레거시로 들어와도 `normal`로 정규화됩니다 — 혼선의 뿌리를 없앤 것이죠. (진짜 screen 합성이 필요하면 하이라이터용으로 새로 구현해야 하는 백로그 항목입니다.)
...
아래 ── 3000 얼굴 바탕 (파운데이션·블러셔)
        3007 하안검 (애교살 포함) · 3009 눈 계열 (홍채, 아이라이너…)
        3019 치아 → 3020 립 → 3022 립 라이너
        3400 조명 보정
위 ──── 4000 안내선
```

> **2026-07-18 변경:** 애교살 전용 렌더러/셰이더가 삭제돼 **하안검(LowerLid, 3007) 안으로 흡수**됐고, 그 아래 큐가 −1씩 재번호됐습니다(치아 3019·립 3020·립라이너 3022). 겹침 관계(치아<립<라이너)는 그대로입니다.
...
A. 정적 분석으로 확정된 사실이고, **2026-07-18 재동기화 후에도 여전히 죽어 있습니다.** 일반 AR 화면이 보내는 화장 적용 메시지는 Fable 쪽 수신자가 **구독 해제된 상태**라 아무도 받지 않습니다(켜 주는 스위치는 튜토리얼 화면 경로에만 있음). 그런데도 화면이 멀쩡해 보이는 건 E3가 대신 그리고 있기 때문입니다. "Fable 쪽 값을 바꿨는데 반응이 없다"면 코드가 아니라 **이 경로 자체가 죽어 있는지**부터 확인하세요.

> ⚠️ **헷갈리기 쉬운 점.** 이번 동기화로 "화장 적용 직전 이전 오버레이를 지우는" 코드(`ClearForkRecipeOverlays`)가 생겼지만, 이건 **메시지 핸들러 안**에 있습니다. 핸들러 자체가 구독 해제로 안 불리니, 이 정리 코드도 일반 AR 화면에선 **실행되지 않습니다.** "정리 코드가 생겼다 = 수신자가 생겼다"가 아닙니다.
...
**Q. ⓝ AR 화면을 나갔다 돌아오면 렌더가 어떻게 되나요? (2026-07-18 신규)**

A. 이제 **"보이는 Unity 화면이 있으면 실행, 없으면 pause"** 로 소유권이 정리됐습니다. AR을 벗어나면 카메라·추론(MediaPipe)·`ARSession`이 멈추고(모델은 warm 유지) 배터리·발열·카메라 점유가 줄며, 돌아오면 **producer→consumer 순서**(세션→프레임브로커→랜드마크→세그멘테이션→스텐실)로 되살아나고 준비완료(readiness) 신호를 회복합니다. 렌더 *결과*가 아니라 **"언제 렌더가 도는가"** 를 바꾼 변경이라, "복귀 직후 한 박자 비어 보인다"면 이 재개 순서/타이밍을 의심하세요. (스텐실 호스트의 활성 판정도 라우트 요청 플래그와 런타임 게이트 둘의 AND로 바뀌었습니다.)
...
**Q. 립은 사용자별 맞춤 마스크로 그리는 것 아니었나요?**

A. 맞춤형은 립을 그리는 **세 방식 중 하나**일 뿐입니다.

| 방식 | 도안 | 어디서 쓰이나 |
|---|---|---|
| Fable 링 메시 | **마스크 자체가 없음** — 매 프레임 랜드마크로 경계를 직접 만듦 | 일반 포인트 메이크업 + 레시피 립(우회) |
| E3 공용 립 마스크 | 미리 그린 PNG — 지도 위 "평균 입술" 도안 | E3 립의 원래 방식 |
| E3 맞춤 생성 마스크 | 당신 얼굴의 랜드마크로 폰에서 그림 | 개인화 립 검증 경로 |

맞춤형은 공용 마스크의 "평균 입술" 문제를 보완하려고 나중에 추가된 것이고, 그마저도 배치 문제(아래 문답) 때문에 지금은 링 메시 우회가 대세입니다. 어느 경로로 들어왔느냐에 따라 셋 중 무엇이 그려지는지가 다릅니다.
...
맞춤 생성 마스크는 ①만 풉니다 — 당신 사진으로 만들었으니 모양은 맞지만, 얹는 건 여전히 색을 못 보는 그물입니다. 맞춤 양복을 지었는데 입혀 주는 사람이 어깨선을 못 잡는 격이죠. 링 메시는 **매 프레임 당신 입술 경계점을 직접 이어** 그 자체가 경계가 되므로 ①②를 한 방에 풉니다. 립이 Fable로 우회 중인 이유입니다.

> **2026-07-18 변경.** 예전 Fable 립에는 링 메시 위에 다시 "붉은기 급락 경계"를 픽셀로 찾아 외곽을 스냅하던 포크 보정이 있었는데, 이번에 **제거**되고 순수 랜드마크 지오메트리로 정리됐습니다. 입꼬리·회전에서 과보정 흔들림이 줄고 경계 계산이 예측 가능해집니다("맞춤 립이 미세하게 떨린다"의 원인 후보 하나가 사라진 셈).
...
A. 광택은 색이 아니라 **빛**입니다(가산 합성). 구현은 **경로마다 다릅니다** — E3 지도-마스크 립(`SmoothRegionMask`)은 립을 그린 뒤 그 위에 하이라이트를 한 번 더 얹는 **2단 패스** 구조라, 광택이 켜지면 그리기 비용이 두 배가 됩니다. 반면 지금 립의 주 경로인 **Fable 링 메시 립(`Lip.shader`)은 단일 패스**로, 광택을 프리멀티 색에 **내부 가산**합니다(별도 패스 없음). "광택 켜면 두 배"는 E3 립에만 해당합니다.
...
| G | **그릴 곳** — 새 눈썹의 몸통 |
| B | **털 결** — 가느다란 스트로크 디테일 |
| A | 셰이더는 안 읽지만 **CPU 삼각형 컬링이 G/A를 샘플** — 죽은 칸 아님 (2026-07-17 정정) |

A 칸은 셰이더 효과에는 안 쓰이지만 CPU가 '이 삼각형에 마스크가 있나'를 판단할 때 씁니다 — 비우면 눈썹 삼각형이 잘려 나갈 수 있습니다.
...
| 스텐실 활성 판정 | `ApplyState()` = `_stencilRequestedActive && _runtimeActive` (옛 `ApplyActive`) | `AuraMediaPipeGraftBootstrap.cs` |
| AR 밖 런타임 pause | `SetRuntimeActive` / `SetLiveProcessingActive` / `SetLiveRuntimeActive` | 부트스트랩 · `FaceLandmarkSource` · `RNBridge` |
| 애교살(흡수) | `_Aegyo*` 속성 (전용 렌더러 없음) | `LowerLid.shader` / `LowerLidRenderer.cs` |

## 부록 B. 끊긴 손잡이 목록 — 돌려도 안 먹는 것들

가설을 세우기 전에 확인하세요. 여기 있는 건 **수리 전에는 실험 무의미**입니다.

> **2026-07-18 재검증.** upstream 대규모 재동기화(113파일) 이후에도 이 목록의 상태는 **전부 그대로**입니다 — 재동기화가 새로 고치거나 깬 손잡이는 없습니다. 참고로 이번 dev의 실제 변화는 손잡이가 아니라 **가동 방식**입니다: 애교살 전용 렌더러 삭제→하안검 흡수(큐 −1 재번호), 립 경계 픽셀 스냅핑 제거, FaceMakeup 제형 스튜디오 추가, AR 밖 Unity 런타임 pause 도입.

| # | 손잡이 | 실태 |
|---|---|---|
| 1 | 눈썹 제거 강도 2종 (모바일 → 셰이더) | ✅ **명시적 무효화 완료(2026-07-17)** — 읽힌 적 없는 셰이더 선언과 세터를 제거. 발신은 스키마 유지용으로 남아 있고, 강도 조절 기능 자체는 여전히 없음(구현하려면 셰이더부터) |
| 2 | `screen` 블렌드 모드 | ✅ **수리됨(2026-07-17)** — 두 계약에서 제거, 레거시 payload는 normal로 별칭 |
| 3 | softness 슬라이더 (클래식 블러셔·립) | 미리 그린 PNG가 있어 무시됨 (미해결 — 제품 결정 대기) |
| 4 | 일반 AR 화면의 Fable `applyFilter` | 수신자 구독 해제 상태 — E3가 대신 그려 티가 안 남 (미해결 — 설계 결정 대기). **2026-07-18 재확인: 여전히 죽음.** 신설 `ClearForkRecipeOverlays`는 핸들러 안이라 수신자가 아님. 옛 심볼 `ApplyActive`→`ApplyState`로 개명 |
| 5 | 립 맞춤 마스크 크기 검증 | ✅ **수리됨(2026-07-17)** — 눈썹처럼 거부 + RN이 터미널 사유 수신 시 재시도 중단 |
| 6 | 생성 눈썹 마스크의 A 채널 | ⚠ **정정** — 셰이더는 안 읽지만 **CPU 삼각형 컬링이 `max(R, G, A)`를 샘플**함(2026-07-18 재확인). "죽은 칸" 아님 — 계속 기록해야 함 |
| 7 | "의미 분할 켜기" | 켜는 문제가 아니라 피부 클래스 자체가 미구현 (미해결) |
| 8 | **F9(신규): 화면공간 파운데이션의 손 가림** | 손 유니폼을 넣는 유일한 코드가 호출자 0 — **기본 파운데이션 경로는 손 가림이 상시 꺼진 채 렌더 중**. 배선은 E7 소유권 정리가 선행돼야 함 |
| 9 | **손 가림의 Fable 립 배선 (구 HF5)** | Codex 리뷰 BLOCKER로 드롭 — E7은 입 ROI 없이는 즉시 실패하는데 Fable 우회 경로는 ROI를 공급하지 않고, 좌표 변환도 비호환. 별도 과제(ROI 공급 설계 + 명시적 공유 변환 + 실기기 검증) |
