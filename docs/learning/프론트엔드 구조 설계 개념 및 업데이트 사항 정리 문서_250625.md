...
| --- | --- |
| 기능 단위 폴더 구조 | `features/image-analysis`, `features/makeup-feedback`, `features/reference-makeup-extraction` |
...
비전공자에게는 "부서별 파일 정리"라고 이해하면 쉽다. 이미지 분석 기능 파일은 image-analysis에, 메이크업 피드백 기능 파일은 makeup-feedback에, 레퍼런스 기반 메이크업 추출 기능 파일은 reference-makeup-extraction에 둔다.
...
- Route params: 특정 화면에 필요한 작은 값을 route로 넘긴다. 예: `MakeupCorrectionTip`의 `pointId`.

### 5. Route type

`routeTypes.ts`는 route 이름과 parameter의 타입 계약이다.

예를 들어 `MakeupCorrectionTip`은 `{pointId: string}`이 필요하다. 타입으로 선언해두면 잘못된 이동 코드를 TypeScript가 잡아줄 수 있다.
...
ARFilter: fullscreen
ExtractedMakeupStyleSaveComplete: fullscreen terminal
```
...
예를 들어 메이크업 피드백 결과 전체, 선택한 사진 객체, 저장된 메이크업 스타일 preview는 URL에 넣기 어렵다. 이런 값은 `NavigationFlowStateProvider`에서 관리한다.
...
aiarmakeup://makeup-correction-tip/eyeline-point
```
...
| flow/context state | 여러 화면을 지나며 유지해야 하지만 URL에 넣기 어려운 값 | `selectedReferenceMakeupPhoto`, `makeupFeedbackResult` |
...
예를 들어 `ImageAnalysisReportDetail`은 `reportId`만 있으면 어떤 보고서를 열지 알 수 있다. `MakeupCorrectionTip`은 `pointId`만 있으면 어떤 수정팁인지 찾을 수 있다. 이런 값은 딥링크와도 잘 맞는다.
...
이 문서는 현재 브랜치 `refactor/mobile-linking-demo-flow-state-0625`와 `dev` 브랜치의 모바일 프론트엔드 구조 차이를 설명한다.

- 현재 브랜치: `47440f1 refactor: 메이크업 스타일 네이밍 정리`
- 비교 대상 dev: `10182ae Merge pull request #55 from devhyun05/feat/asset-flow-refactor`
- 최근 네이밍 정리 반영: `1d67875` 메이크업 추출, `c8c2e35` 메이크업 피드백, `47440f1` 메이크업 스타일
- 주요 비교 범위: `apps/mobile/src`, `apps/mobile/package.json`, `apps/mobile/package-lock.json`, `docs/mobile`
- 초기 React Navigation 구조 비교 변경 규모: 155개 파일 변경, 9,854줄 추가, 7,116줄 삭제
...
AppRoot.tsx
  - activeScreen 상태 보관
  - selectedPhoto, makeupFeedbackResult, selectedReferenceMakeupPhoto 등 흐름 상태 보관
...
- 좋아요 목록
- 메이크업 스타일 목록
- 메이크업 피드백 시작/로딩/결과/수정 가이드/수정팁
- 레퍼런스 메이크업 추출 업로드/결과/스타일 저장/레시피 상세
...
`dev`에서는 `UserPageScreen`, `FavoriteProductsScreen`, `LikedProductListScreen`, 과거 `MakeupLookScreen`, 현재의 `MakeupStyleListScreen` 성격 화면 등이 profile feature 안에 있었다.
...
최근 네이밍 정리의 기준은 "사용자가 이해하는 제품 언어"와 "코드가 맡은 역할"을 맞추는 것이다.

- 사용자에게 저장, 추천, 추출 결과로 보이는 메이크업 단위는 `Look/룩`이 아니라 `Style/스타일`로 부른다.
  - 타입과 데이터: `MakeupStyle`, `MakeupStylePreview`, `makeupStylesMock`
  - 변수: `savedMakeupStyle`, `extractedMakeupStyle`, `recommendedStyles`
  - 화면 문구: `메이크업 스타일`, `추천 스타일`, `저장한 스타일`
- 레퍼런스 이미지에서 메이크업 정보를 분석해 추출하는 플로우는 `ReferenceMakeupExtraction`으로 부른다.
  - 화면: `ReferenceMakeupExtractionUpload`, `ReferenceMakeupExtractionResult`
  - 재사용 추출 로직: `MakeupExtraction`, `makeupExtractionService`
  - 선택 사진 상태: `selectedReferenceMakeupPhoto`
- 현재 메이크업 사진을 분석해 점수, 강점, 수정 포인트를 주는 플로우는 `MakeupFeedback`으로 부른다.
  - 화면: `MakeupFeedbackEntry`, `MakeupFeedbackResult`
  - 결과 상태: `makeupFeedbackResult`
  - 수정 방법 화면: `MakeupCorrectionGuide`, `MakeupCorrectionTip`
- `Filter`는 실제 AR 필터, 필터 적용, 필터 생성, 프리셋 의미일 때만 남긴다.
  - 유지 예: `ARFilter`, `ARFilterLocationAdjust`
  - 미래 이름 후보: `MakeupFilterGeneration`, `MakeupFilterPreset`
- React Native의 `style` prop과 `StyleSheet`의 `styles` 객체는 도메인 `Style`과 무관하므로 바꾸지 않는다.
- 영어 동사 `look` 의미는 바꾸지 않는다. 예를 들어 `isLookingForward` 같은 이름은 유지한다.
- 이미지 asset의 `look-*.png`는 import 영향이 커서 별도 asset 정리 작업으로 다룬다.

새 이름을 정할 때는 넓은 단어 하나만 쓰기보다 도메인과 역할을 함께 드러낸다. 예를 들어 단독 `FeedbackResult`보다 `MakeupFeedbackResult`가 낫고, 추출 결과를 `filterResult`라고 부르기보다 `extractedMakeupStyle`이라고 부르는 편이 다음 기능과 충돌하지 않는다.

## 20. 새 화면을 추가할 때의 추천 순서

아래 순서를 따르면 현재 구조와 충돌할 가능성이 낮다.

1. 화면이 어떤 기능인지 정한다.
   - 예: image-analysis, makeup-feedback, reference-makeup-extraction, profile, recommendation
