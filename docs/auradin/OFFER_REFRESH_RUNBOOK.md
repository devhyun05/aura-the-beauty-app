...
### 4.1.1 판정 에스컬레이션 (애매한 항목 처리 규칙)

CSV만 보고 판단하지 않는다. 애매한 항목은 아래 순서로 증거를 확보한 뒤 기입한다:

1. **네이버 API 재조회**: 해당 상품 질의로 현재 리스팅들을 다시 받아 시세 범위·전체 제목·중량/용량(0.05g vs 0.5g 등 변형 구분)·세트/리필 마커를 대조한다. 매칭 오퍼가 네이버 가격비교 카탈로그 페이지(`search.shopping.naver.com/catalog/...`)면 그 링크는 항상 열리므로 직접 확인한다.
2. **외부 검색**: 그래도 애매하면 구글링·브랜드 공식몰 등에서 **정확한 상품명과 사양**을 찾아 확정하고, note에 근거와 함께 기입한다.
3. **확정 불가**: 위 둘로도 확정이 안 되면 `keep_old` + note에 "확정 불가 사유"를 남기고 다음 run에서 재검토한다.

주의: 판정 화면·가이드에서 상품명을 절단해 보여주지 않는다 — 중량/호수/구성(끝부분 숫자)이 판정의 핵심 단서다(사례: "…엑스퍼트 0.05g"가 "0.0"으로 잘려 오독). 검토 큐 항목의 원 매칭 증거는 `results.jsonl`의 `matchedOffer`(전체 제목·몰·productId)에서 꺼내 대조한다.
