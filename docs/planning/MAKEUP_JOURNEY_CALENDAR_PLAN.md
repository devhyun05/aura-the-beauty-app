...
- 하루 대표 점수는 사용자가 날짜 상세에서 선택한 완료 피드백 점수다. 선택 전에는 가장 최근 완료 피드백을 기본 대표값으로 사용하며 점수 자체는 복제하지 않는다.
- 성공/실패는 `대표 점수 >= 현재 목표 점수`로 조회 시점에 계산한다. 목표 점수를 바꾸면 과거 날짜도 새 기준으로 즉시 재평가한다.
- 첫 캘린더 진입 때 개인 목표 점수(1–100)만 받는다. 미션 난이도는 설정하지 않고 매일 목적이 다른 세 가지 미션을 한 세트로 제공한다.
- 수정 메이크업은 기존 피드백 흐름을 재사용하고 원본 리포트와 같은 날짜에 연결한다. 기존 피드백 목표 문맥은 상속해 목표 입력 화면을 건너뛴다.
- 월간 셀은 사용자가 선택한 대표 리포트(선택 전에는 최신 완료 리포트)의 얼굴 썸네일을 배경으로 표시하고, 좌측 상단 날짜와 하단 반투명 오버레이에 대표 점수·달성 여부를 함께 표시한다. 날짜 상세는 사진 슬라이드의 현재 리포트를 규칙 기반으로 줄인 `한눈 요약`과 점수를 함께 표시한다.
...
운동 앱 레퍼런스에서 가져오는 것은 넓은 월간 격자, 날짜별 상태를 한눈에 구분하는 방식, 선택 날짜의 상세 진입 구조다. 영양소·생리 주기·체중 대시보드 등 앱과 무관한 정보는 가져오지 않는다. 기록이 있는 캘린더 셀은 대표 얼굴 썸네일과 점수·성공/실패만 담고, 긴 피드백과 미션은 날짜 상세로 분리한다.
...
2. 상세 상단의 `이전 날`과 `다음 날`을 눌러 달력으로 나가지 않고 과거·미래 날짜를 하루씩 계속 이동한다.
3. 날짜 상세 화면 어디서든 좌우로 스와이프해 각 촬영 시점의 사진, 점수, `한눈 요약`, 일기 & 메모를 함께 전환한다. 세로 스크롤은 기존대로 카드 탐색에 사용한다.
4. 이전 날/다음 날 사이의 점수 선택 버튼으로 현재 사진의 점수를 이날의 대표 점수로 저장한다.
5. 더 자세한 근거가 필요하면 `전체 AI 보고서 보기`, 기간별 변화를 보려면 `그래프 보기`를 누른다.
6. 아래로 스크롤해 날짜 단위로 공유되는 미션과 수정 메이크업, 현재 리포트에 속한 일기 & 메모를 확인한다.
7. `수정 메이크업 업로드`를 누르면 기존 카메라/앨범 선택 흐름으로 이동한다.
8. 선택한 날짜와 최신 리포트 ID, 상속할 피드백 목표 문맥을 네비게이션 컨텍스트에 넣는다.
9. 목표 입력 화면을 생략하고 기존 분석·폴링·결과 화면을 사용한다.
10. 완료된 수정 리포트는 같은 `entryDate`에 `feedbackKind=correction`으로 저장되고 상세·그래프가 갱신된다.
...
| 연·월 빠른 이동 | 현재 `YYYY년 M월` 제목 | 제목을 눌러 연도·월 2열 휠을 스크롤하고 완료 시 이동(일 선택 제외) |
| 요약 | 월 평균, 현재 목표, 연속 기록 | 완료된 대표 점수만 집계 |
| 요일/격자 | 일–토, 6주 고정 격자 | 월 바깥 날짜는 흐리게 표시 |
| 날짜 셀 | 좌측 상단 날짜, 대표 얼굴 썸네일, 하단 점수·성공/실패 오버레이 | 탭 시 날짜 상세 |
| 범례 | 목표 달성, 목표 미달, 기록 없음 | 색 외에 체크/실패 표식 병행 |
| 설정 | 목표 점수 수정 | 저장 후 전체 월 재평가/갱신 |

상태 규칙:

- `success`: 대표 점수가 현재 목표 이상
- `failure`: 대표 점수가 현재 목표 미만
- `empty`: 완료 점수 없음
- `today`: 기기 로컬 날짜를 검은 날짜 배지로 표시하고 탭 재진입과 자정에 자동 갱신
- 대표 얼굴 썸네일은 사용자가 고른 대표 리포트를 우선하고 선택 전에는 최신 완료 리포트를 사용한다. 월 응답에는 원본 이미지나 전체 보고서 payload를 넣지 않고 인증이 필요한 앱 내부 썸네일 전달 경로만 포함한다. 전달 API는 현재 사용자 소유의 완료 리포트인지 다시 확인하고 S3의 후처리 썸네일만 `private, no-store`로 응답한다.
- 썸네일이 없는 과거 기록은 중립 배경에 점수·상태를 유지하고 원본 대용량 이미지를 월 화면에서 내려받지 않는다.
- 월 평균은 날짜별 대표 점수의 산술평균을 반올림한다.
- 월간 요약은 흰색 리포트 카드에서 평균 점수, 목표 차이, 진행률, 기록일, 연속 기록 순으로 보여 주며 장식용 영문 배지나 중첩된 검정 카드 패널은 사용하지 않는다.
...
화면은 고정 헤더, 인접 날짜 이동 바, 보고서 단위의 가로 페이지와 각 페이지 안의 세로 `ScrollView`로 구성한다. 손가락을 좌우로 드래그하는 동안 옆 보고서가 함께 나타나는 네이티브 페이징을 사용하고, 페이지가 정착하면 사진·점수·요약·메모와 상단 점수 선택 상태를 같은 `reportId`로 갱신한다. 고정 헤더는 왼쪽 `달력으로`, 가운데 날짜, 오른쪽 `그래프 보기`다. 그 아래의 `이전 날`과 `다음 날`은 현재 루트 화면의 `entryDate`만 교체해 스택을 늘리지 않고 과거·미래 날짜를 하루씩 계속 이동한다. 미래 날짜는 기존 정책대로 조회만 허용한다. 두 번째 이후 보고서에서는 이전 보고서 방향의 스와이프가 가로 페이징으로만 동작하도록 iOS 뒤로가기 제스처를 끄고, 첫 번째 보고서에서는 더 이전 방향으로 미는 동작이 인터랙티브한 화면 뒤로가기로 이어지도록 다시 켠다. 명시적인 `달력으로` 버튼은 뒤로 갈 스택이 있으면 `goBack()`, 딥링크 등으로 스택이 없으면 해당 월의 `MakeupJourneyTab`으로 이동한다.

스크롤 노출 순서:

1. 촬영 시점별 사진 슬라이드
2. 현재 사진의 점수·성공/실패·최초 대비 변화량
3. 현재 사진의 AI 피드백 `오늘의 한눈 요약`
4. 오늘의 미션
5. 수정 메이크업
6. 일기 & 메모
...
3. `strengths`의 모든 제목과 전체 개수
4. `points`의 모든 제목과 전체 개수
5. 첫 보완점의 `actionSteps[0]`, 없으면 `description`을 `먼저 해볼 것`으로 표시
6. `전체 AI 보고서 보기`로 기존 `MakeupFeedbackResult(reportId)` 이동

요약은 **각 완료 리포트별로** 서버가 읽기 시점에 만든다. 날짜 상세 화면의 가로 스와이프로 활성 리포트가 바뀌면 같은 `reportId`의 사진·점수·요약·메모를 함께 바꾼다. 새 모델 호출, 새 점수 산정, 요약문 DB 복제는 하지 않는다. 과거 리포트에 일부 필드가 없으면 존재하는 항목만 렌더링한다.

세부 규칙:

- 리포트가 하나면 현재 사진 점수와 최초 대비 변화량을 표시한다.
- 리포트가 둘 이상이면 `최초 76 → 현재 84 (+8)`처럼 현재 슬라이드 기준으로 표시한다.
- 월간 셀에는 긴 글을 넣지 않는다. 대표 얼굴 썸네일 위에 점수·성공/실패만 짧게 표시하고 정리된 글은 날짜 상세에서 제공한다.
- `전체 AI 보고서 보기`는 기존 `MakeupFeedbackResult`로 현재 슬라이드의 `reportId`를 전달한다.
- 수정 CTA는 최신 완료 리포트를 부모로 사용하되 모든 수정은 같은 최초 기록 체인에 묶는다.
- 메모는 리포트별로 저장하며 2,000자 제한, 자동 저장이 아닌 명시적 저장 방식으로 중복 요청을 줄인다. 리포트가 없는 날짜에는 날짜 단위 메모를 허용한다.
- 미션 완료 체크는 낙관적으로 반영하고 실패 시 원복과 토스트를 표시한다.
- 미션은 리포트별로 복제하지 않고 `entryDate` 단위로 모든 슬라이드에서 공유한다.
...
- 날짜별 대표 점수를 선으로 연결하고, 현재 목표 점수를 수평 점선으로 표시한다.
...
- 미션 난이도는 설정하지 않는다. 데일리 미션은 항상 세 가지 목적을 한 세트로 추천한다.
...
- `id uuid primary key`, `user_id` FK, `entry_date date`, nullable `report_id` feedback report FK, `content text`
- `check (char_length(content) <= 2000)`
- 리포트 메모는 partial unique `(user_id, entry_date, report_id) where report_id is not null`
- 기록 없는 날짜의 메모는 partial unique `(user_id, entry_date) where report_id is null`
- 기존 날짜 단위 메모는 같은 날짜의 최신 완료·점수 보유 리포트로 안전하게 backfill한다. 연결할 리포트가 없으면 `report_id`를 null로 유지한다.
- `created_at`, `updated_at`

`makeup_journey_day_score_selections`

- `(user_id, entry_date)` 복합 기본 키, `report_id`는 `makeup_feedback_reports(id)` FK
- 선택 행과 리포트 모두 `on delete cascade`; 점수를 복제하지 않고 대표 리포트 ID만 저장
- API가 리포트의 사용자 소유권, 날짜, 완료 상태, 점수 존재 여부를 확인한 뒤 upsert
- `report_id` 조회 인덱스, `created_at`, `updated_at`
  ...
  "days": [
    {
      ...
      "firstScore": 76,
      "latestScore": 84,
      "representativeReportId": "uuid",
      "representativeScore": 76,
      "representativeThumbnailUrl": "/makeup-journey/reports/uuid/thumbnail",
      "scoreDelta": 0,
      "reportCount": 2,
      "hasNote": true,
...
응답은 `date`, `status`, `goalScore`, `firstScore`, `latestScore`, `representativeReportId`, `representativeScore`, `scoreDelta`, 대표 `feedbackDigest`, 시간순 최소 정보 `reports`, 날짜 공유 `missions`, 호환용 대표 `note`를 포함한다. 각 `reports[]`는 해당 리포트의 `feedbackDigest`와 리포트별 `note`를 포함한다. 다른 사용자 리포트 ID나 날짜는 404로 숨긴다.
...
`feedbackDigest`는 `feedback_payload.result`에서 allow-list 필드만 읽어 조립한다. 전체 리포트 payload와 원본 이미지 URL을 월간 응답에 싣지 않으며, 월간 셀에는 대표 리포트 ID 기반의 인증된 후처리 썸네일 전달 경로만 허용한다. 모바일은 인증 헤더를 포함하고 메모리 전용 캐시를 사용하며 계정 변경 시 private image generation과 네이티브 메모리 캐시를 초기화한다.

### 날짜 대표 점수 선택

```http
PUT /makeup-journey/days/2026-07-17/score-selection
Content-Type: application/json
...
- 각 날짜의 대표 점수를 포함하며, 사용자가 선택하지 않은 날짜는 최신 완료 점수를 사용한다. 빈 날짜를 0점으로 채우지 않는다.
...
- note body: `{"content":"...","reportId":"uuid"}`; 빈 문자열은 해당 리포트 메모 삭제와 동일하게 처리한다. 리포트 없는 날짜는 `reportId:null`을 사용한다.
...
검수된 로컬 미션 풀의 무비용 랜덤 추천 방식을 사용한다.

1. **기본 풀:** 세 목적별로 검수된 미션 10~20개를 버전 관리된 상수로 제공한다.
2. **랜덤 추천:** 각 목적의 후보에서 하나씩 무작위로 고르되 다른 날짜의 최근 자동 추천 45개와 같은 날짜의 기존 미션은 제외한다. 날짜를 탐색한 순서와 관계없이 중복을 막는다.
3. **사용자 추가:** 사용자가 개인 미션을 자유롭게 만든다.

화면은 생성 시점을 보존한 `sortOrder`를 따른다. 데일리 미션 추천에서는 AI를 호출하지 않으므로 비용과 네트워크 지연이 없고, 원본 얼굴 사진·피드백 payload·메모도 추천 입력으로 사용하지 않는다.

미션은 메이크업 기술 교정 과제가 아니라 사용자가 `오늘 한 번 해볼까?`라고 느낄 수 있는 짧은 뷰티 습관이다. 메이크업 추출, 저장한 메이크업, 메이크업 피드백, 얼굴 분석, 아우라딘 같은 앱 기능과 메이크업·스킨케어·가벼운 웰니스 행동을 함께 추천한다.

- **가벼운 습관:** 준비 없이 1~5분 안에 바로 할 수 있는 행동 하나.
- **뷰티 루틴:** 앱 기능 체험 또는 5~15분 정도의 관리 행동 하나.
- **오늘의 도전:** 점수·완성·사진처럼 결과나 기록을 남기는 행동 하나.

`추천 받기` 한 번에 위 세 가지를 정확히 하나씩 생성한다. 사용자는 난이도를 고르지 않으며 세 미션을 모두 오늘의 데일리 미션으로 받는다. DB/API의 기존 `beginner/intermediate/advanced` 값은 각각 세 목적을 구분하는 내부 호환 키로만 유지한다.

미션 제목에는 한 행동만 담고 3~24자의 친근한 일상어를 사용한다. 이유, 준비물 설명, 세부 순서, 여러 완료 기준을 제목 한 문장에 합치지 않으며 모든 미션을 기술 훈련으로 만들지 않는다.
...
- 월간 쿼리는 N+1 없이 리포트·미션·리포트별 메모를 월 범위 집계한다.
...
- 그래프 API는 월간 집계와 같은 날짜별 대표 점수(선택값, 없으면 최신값) CTE를 재사용한다.
- 목표 변경 전후 동일 월을 조회해 상태만 재평가되고 점수는 변하지 않는지 검증한다.
- 날짜 상세의 각 report digest가 같은 `reportId`의 기존 결과와 일치하는지 계약 테스트를 둔다.

### M3–M4 — 모바일 핵심

- 네비게이션/하단 바를 먼저 연결하되 API 실패가 다른 탭 렌더링을 막지 않게 한다.
- 월 캐시는 `YYYY-MM` 키로 분리하고 설정 변경·피드백 완료·미션/메모 저장 뒤 관련 월/일을 무효화한다. 현재 월 로드 뒤 이전·다음 월 데이터와 대표 썸네일을 동시성 제한된 백그라운드 작업으로 미리 받고, 계정/캐시 무효화 revision이 바뀐 이전 요청은 캐시에 다시 쓰지 않는다.
- 상세는 고정 헤더 + 보고서 가로 페이저 + 페이지별 단일 세로 `ScrollView`로 만들고 카드별 최소 높이·여백을 지켜 한 화면 압축을 금지한다.
...
- 같은 날짜 대표 리포트 선택의 소유권·날짜·완료 상태 검증과 월간/그래프 반영
- 같은 날짜 대표 리포트 선택 시 월간 대표 리포트 ID·썸네일이 함께 바뀌고 선택 전에는 최신 완료 리포트 썸네일을 쓰는지 검증
...
- 리포트별 메모 upsert/삭제와 소유권, 기록 없는 날짜 메모, 미션 중복/완료 일관성, AI 폴백

### 모바일

- 탭 순서와 접근성 라벨, 선택 상태
- 첫 진입 설정, 빈 월, 기록 월, API 오류와 재시도
- 대표 얼굴 썸네일 셀, 이미지 없는 중립 폴백, 좌측 날짜, 점수·달성 여부 오버레이와 접근성 라벨
- 현재·인접 월 썸네일 prefetch 동시성 제한, 중복 제거, 캐시 무효화/계정 경계
- 월 변경 중 늦게 도착한 응답이 현재 월을 덮지 않음
- 날짜 상세 단일/복수 리포트 렌더링, 상세 화면 전체 가로 스와이프에 따른 사진·점수·요약·메모 동기화와 세로 스크롤 보존
- 인접 날짜 사이 대표 점수 선택 버튼의 저장·선택 상태·오류 복구
...
- `makeup_journey_settings_saved` — goalScore (`missionLevel`은 하위 호환 필드이며 사용자 선택값이 아님)
...
- 날짜 상세에서 사진별 원본과 일치하는 한눈 요약을 읽고 해당 전체 보고서로 이동할 수 있다.
- 날짜별 대표 점수를 직접 선택하면 월간 상태·평균·그래프에 같은 값이 반영된다.
