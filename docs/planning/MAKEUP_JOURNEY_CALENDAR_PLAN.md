...
3. 최초·최신 점수, 변화량과 최신 리포트의 `한눈 요약`을 먼저 읽는다.
4. 더 자세한 근거가 필요하면 `전체 AI 보고서 보기`, 기간별 변화를 보려면 `그래프 보기`를 누른다.
5. 아래로 스크롤해 미션, 수정 메이크업, 일기 & 메모를 확인한다.
6. `수정 메이크업 업로드`를 누르면 기존 카메라/앨범 선택 흐름으로 이동한다.
7. 선택한 날짜와 최초 리포트 ID, 상속할 피드백 목표 문맥을 네비게이션 컨텍스트에 넣는다.
8. 목표 입력 화면을 생략하고 기존 분석·폴링·결과 화면을 사용한다.
9. 완료된 수정 리포트는 같은 `entryDate`에 `feedbackKind=correction`으로 저장되고 상세·그래프가 갱신된다.
...
화면은 고정 헤더, 인접 날짜 이동 바, 하나의 세로 `ScrollView`로 구성한다. 고정 헤더는 왼쪽 `달력으로`, 가운데 날짜, 오른쪽 `그래프 보기`다. 그 아래의 `이전 날`과 `다음 날`은 현재 루트 화면의 `entryDate`만 교체해 스택을 늘리지 않고 과거·미래 날짜를 하루씩 계속 이동한다. 미래 날짜는 기존 정책대로 조회만 허용한다. 뒤로 갈 스택이 있으면 `goBack()`, 딥링크 등으로 스택이 없으면 해당 월의 `MakeupJourneyTab`으로 이동한다.
