...
COLLECTOR_VERSION = "auradin-chicor-metadata-v1"
...
def _extract_title(page_text: str, fallback: str) -> str:
  title_input = re.search(r'<input[^>]+id=["\']contText["\'][^>]+value=["\']([^"\']+)["\']', page_text, flags=re.I)

  if title_input:
    return _squash(html.unescape(title_input.group(1)))

  title = re.search(r"<title[^>]*>(.*?)</title>", page_text, flags=re.I | re.S)

  if title:
    return _squash(html.unescape(re.sub(r"<[^>]+>", " ", title.group(1))))

  return fallback


def _decode_js_object_array(raw_value: str) -> list[dict[str, Any]]:
  try:
...
def _extract_option_names(page_text: str) -> list[str]:
  ...
  match = re.search(r"var\s+goosOptnList\s*=\s*(\[.*?\]);", page_text, flags=re.I | re.S)

  if match:
    for item in _decode_js_object_array(match.group(1)):
      name = _squash(item.get("optnNm"))

      if name and name not in seen:
        seen.add(name)
        names.append(name)

  for option_match in re.finditer(r'<option[^>]+class=["\']optnVal["\'][^>]*>(.*?)</option>', page_text, flags=re.I | re.S):
    name = _squash(html.unescape(re.sub(r"<[^>]+>", " ", option_match.group(1))))
  ...
  *,
  ...
  for option_name in _extract_option_names(page_text):
    ...
    options.append(
      _shade_payload_from_name(
        option_name,
        source_type="chicor_product_option_json",
        confidence=0.82,
      ),
    )
...
def _parse_chicor_page(
  ...
  *,
  ...
  shade_options = _extract_chicor_options(
    page_text=page_text,
  ...
  for field, rules, confidence in (
    ...
    if value:
      fields[field] = _field_payload(
        ...
        evidence_text,
        source_url,
        source_type="chicor_product_text",
      )
...
def collect_chicor_metadata(
  *,
  ...
  for row in rows:
    ...
    if _domain(source_url) == "chicor.com":
...
def _write_report(
  ...
  lines = [
    "# Auradin Chicor Metadata Collection",
    "",
  ...
  lines.extend(
    [
      ...
      "## Notes",
      "",
      "- Source is public Chicor product HTML from existing Naver live-offer URLs.",
      "- Only option names from `goosOptnList`/option select and short title evidence are retained.",
...
def parse_args() -> argparse.Namespace:
  parser = argparse.ArgumentParser(description="Collect limited Auradin metadata from public Chicor product pages.")
...
def main() -> int:
  ...
  output_path = args.output or REPO_ROOT / "data" / "auradin" / "detail" / "retail" / f"chicor_metadata_{args.date}.jsonl"
  report_path = args.report or REPO_ROOT / "reports" / "auradin" / f"chicor_metadata_collection_{args.date}.md"
  result = collect_chicor_metadata(
