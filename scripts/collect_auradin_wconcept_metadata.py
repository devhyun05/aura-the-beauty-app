...
COLLECTOR_VERSION = "auradin-wconcept-metadata-v1"
COUNTRY_MAP = {
  "대한민국": "Korea",
  "한국": "Korea",
  "국내": "Korea",
...
def _extract_title(page_text: str, fallback: str) -> str:
  ...
  return _squash(html.unescape(re.sub(r"<[^>]+>", " ", match.group(1))).replace("W컨셉(W CONCEPT)", ""))


def _extract_color_options(
  *,
  page_text: str,
  product_name: str,
  ...
  for block_match in re.finditer(r"<dt>\s*색상\s*</dt>\s*<dd>(.*?)</dd>", page_text, flags=re.I | re.S):
    block = block_match.group(1)

    for option_match in re.finditer(r"<option\b([^>]*)>(.*?)</option>", block, flags=re.I | re.S):
      attrs = option_match.group(1)
      visible_text = _squash(html.unescape(re.sub(r"<[^>]+>", " ", option_match.group(2))))

      if "선택" in visible_text:
        continue

      attr_value_match = re.search(r'optionvalue=["\']([^"\']+)["\']', attrs, flags=re.I)
      option_name = _squash(html.unescape(attr_value_match.group(1) if attr_value_match else visible_text))

      if not option_name or not _plausible_structured_option(option_name, product_tokens, brand_aliases):
        continue

      options.append(
        _shade_payload_from_name(
          option_name,
          source_type="wconcept_color_option_select",
          confidence=0.8,
        ),
      )
...
def _extract_made_in_country(page_text: str) -> tuple[str | None, str]:
  ...
  match = re.search(
    r"제조사/제조국\s*</span>\s*([^<]+?)\s*/\s*(대한민국|한국|국내|Korea|South Korea|중국|China|일본|Japan|이탈리아|Italy|프랑스|France|미국|USA|United States)",
    text,
    flags=re.I,
  )
  ...
  return _normalize_country(raw_country), f"제조사/제조국 {raw_country}"


def _parse_wconcept_page(
  row: dict[str, Any],
  *,
  page_text: str,
  source_url: str,
  ...
  shade_options = _extract_color_options(
    page_text=page_text,
  ...
  if made_in_country:
    fields["madeInCountry"] = _field_payload(
      ...
      made_in_country_raw,
      source_url,
      source_type="wconcept_product_summary",
    )
  ...
  for field, rules, confidence in (
    ...
    if value:
      fields[field] = _field_payload(
        ...
        evidence_text,
        source_url,
        source_type="wconcept_product_text",
      )
  ...
  return {
    ...
    "wconceptProductName": title,
  }


def collect_wconcept_metadata(
  *,
  ...
  for row in rows:
    ...
    if _domain(source_url) == "www.wconcept.co.kr":
...
def _write_report(
  ...
  lines = [
    "# Auradin W Concept Metadata Collection",
    "",
  ...
  lines.extend(
    [
      ...
      "## Notes",
      "",
      "- Source is public W Concept product HTML from existing Naver live-offer URLs.",
      "- Only color select options and product-summary manufacturing country are retained.",
...
def parse_args() -> argparse.Namespace:
  parser = argparse.ArgumentParser(description="Collect limited Auradin metadata from public W Concept product pages.")
...
def main() -> int:
  ...
  output_path = args.output or REPO_ROOT / "data" / "auradin" / "detail" / "retail" / f"wconcept_metadata_{args.date}.jsonl"
  report_path = args.report or REPO_ROOT / "reports" / "auradin" / f"wconcept_metadata_collection_{args.date}.md"
  result = collect_wconcept_metadata(
