...
export function runFaceRatioConstantsTests() {
  ...
  // pose 결측/비유한값 = 판정 보류(fail-closed). 0°로 간주하면 불확실
  // 구간이 사라져 오히려 단정이 복원되므로(fail-open) 금지 — 1차 리뷰 반영.
  expect(
    judgeFaceLength(1.3, undefined, Number.NaN).verdict === 'indeterminate' &&
      judgeFaceLength(1.3, Number.NaN, 0).verdict === 'indeterminate' &&
      judgeFaceLength(1.3, 0, undefined).verdict === 'indeterminate',
    'Missing/non-finite pose must withhold the verdict (fail-closed), not assume frontal.',
  );

  // ── 정확 경계: band 끝이 경계와 일치하면 단정 쪽으로 닫힌다(<=, >=) ──
  expect(
    judgeFaceLength(FACE_LENGTH_REFERENCE.wide, 0, 0).verdict === 'wide' &&
      judgeFaceLength(FACE_LENGTH_REFERENCE.long, 0, 0).verdict === 'long',
    'Exact-boundary frontal ratios must resolve to the closed side (hi<=wide, lo>=long).',
  );
  expect(
    judgeFaceLength(FACE_LENGTH_REFERENCE.wide + 1e-6, 0, 0).verdict === 'average' &&
      judgeFaceLength(FACE_LENGTH_REFERENCE.long - 1e-6, 0, 0).verdict === 'average',
    'Just inside the bounds (frontal) must judge average.',
  );

  // ── pitch+yaw 동시 적용 시 band 수치 자체 검증 ──
  const both = judgeFaceLength(1.455, 12, 8);
  expect(
    near(both.band.lo, 1.455 * Math.cos((8 * Math.PI) / 180), 1e-12) &&
      near(both.band.hi, 1.455 / Math.cos((12 * Math.PI) / 180), 1e-12),
    'Band endpoints must be m*cos(yaw) and m/cos(pitch) under simultaneous pose.',
  );

  // ── 제목 매핑: 유보 2종 + 보류 포함 전 verdict 커버 ──
  expect(
    getFaceLengthTitle('wide') === '가로형 얼굴' &&
      ...
      getFaceLengthTitle('indeterminate') === '길이 비율 판정 보류',
    'Every verdict must map to its title, including borderline and indeterminate.',
  );
