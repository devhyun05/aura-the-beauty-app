...
// ── 4. blocked 축·geometry 18키 채움·구버전 방어 ─────────────────────────────
{
  ...
  // metrics 가 일부 누락된(부분 손상) 응답도 18키 전부 채워져야 화면이 안 죽는다.
  ...
  expectEqual(FACE_GEOMETRY_METRIC_KEYS.every(key => key in partialGeometry.metrics), true, '18키 전부');
}
...
// 백엔드 camelize 왕복 후에도 18개 geometry 키가 값까지 그대로 복원돼야 한다.
{
  ...
  // DB → 백엔드 camelize → 복원: 18 geometry 키가 값까지 보존.
