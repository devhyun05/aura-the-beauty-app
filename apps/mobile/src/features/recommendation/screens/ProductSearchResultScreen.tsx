...
import {FlatList, Linking, Pressable, StyleSheet, View, type ViewToken} from 'react-native';
...
export function ProductSearchResultScreen({
  ...
  const toggleLike = async (product: CatalogProduct) => {
    ...
    try {
      if (liked) await unlikeProduct(product.productId, product.externalSource);
      else {
        if (product.externalSource) {
          await likeExternalProduct(product.productId, product.externalSource);
        } else {
  ...
  const open = async (product: CatalogProduct, position: number) => {
    if (state.data?.searchRequestId) queueProductEvent({
      eventType: 'search_result_open',
      section: 'search',
      ...productEventIdentity(product),
      searchRequestId: state.data.searchRequestId,
      position,
      context: {screen: 'product_search'},
    });
    if (product.externalSource) {
  ...
  impressionRef.current = (product, position) => {
    ...
    queueProductEvent({
      ...
      searchRequestId: state.data.searchRequestId,
      position,
      context: {screen: 'product_search', viewportRatio: 0.6, visibleMs: 700},
    });
  };
      ...
      <FlatList
        ...
        ListEmptyComponent={state.status === 'loading'
          ...
          : state.status === 'error'
            ...
            : <RecommendationSectionState kind="empty" message="일치하는 판매 상품이 없어요. 브랜드나 제품명을 바꿔 검색해 보세요." />}
        ...
        ListHeaderComponent={<View style={styles.header}><Text style={styles.title}>‘{query}’ 검색 결과</Text><Text style={styles.description}>카탈로그와 판매 정보가 확인되는 상품의 브랜드·제품명·카테고리를 검색해요.</Text></View>}
        numColumns={2}
        onViewableItemsChanged={onViewableItemsChanged}
        renderItem={({item, index}) => <RecommendationProductCard product={item} onOpen={() => {void open(item, index);}} onToggleLike={() => toggleLike(item)} />}
