  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <Text style={styles.updated}>최종 업데이트: 2026-07-13</Text>

        <Section title="1. 카메라 및 얼굴 이미지">
          AURA는 얼굴 촬영, AR 메이크업, 퍼스널 컬러 분석을 위해 카메라를 사용합니다. 퍼스널
          컬러를 포함한 얼굴 측정 계산은 기기 안에서 수행됩니다. 촬영한 사진은 AI 분석 보고서
          생성을 위해 AURA 서버로 전송·저장되며, 보고서 삭제 시 함께 삭제됩니다.
        </Section>
        <Section title="2. 퍼스널 컬러·얼굴 측정 데이터">
          피부·헤어·입술 색에서 추출한 수치(예: Lab 값, 시즌 성향)와 얼굴 비율·입체 측정
          수치는 기기 내에서 계산됩니다. 측정에 사용된 원본 얼굴 프레임은 계산 후 즉시
          삭제됩니다. 계산된 결과 수치(조명 보정 후 값 포함)는 기기에 저장되고, AI 분석
          보고서를 생성할 때 해당 보고서에 첨부되어 AURA 서버에 함께 저장됩니다.
        </Section>
        <Section title="3. 저장 및 삭제">
          측정 결과는 기기 로컬 저장소에 보관되며, 앱 내 “내 색상 데이터 삭제”로 언제든 삭제할
          수 있습니다. 앱 삭제 시 함께 제거됩니다. 서버에 저장된 보고서 첨부 측정 수치는 해당
          분석 보고서를 삭제하면 함께 삭제됩니다.
        </Section>
        <Section title="4. 제3자 제공">
          얼굴 이미지·파생 색 데이터는 보고서 저장을 위한 AURA 서버 외 제3자에게 제공되지
