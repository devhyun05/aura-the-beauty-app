...
export type ReferenceMakeupExtractionCaptureResult = {
  imageUri?: string;
};

type ReferenceMakeupExtractionCaptureScreenProps = {
  onCapture: (result: ReferenceMakeupExtractionCaptureResult) => void;
  onClose: () => void;
};

export function ReferenceMakeupExtractionCaptureScreen({
  onCapture,
  onClose,
}: ReferenceMakeupExtractionCaptureScreenProps) {
  ...
  const handleCapture = async () => {
    ...
    try {
      ...
      // Simulator camera capture can fail while the mock extraction flow remains demoable.
    } finally {
      setIsTakingPhoto(false);
      onCapture({imageUri: capturedImageUri});
    }
  };
  ...
  return (
    <FullscreenOverlayScreen>
          ...
          {permission === null ? (
            ...
            <>
              ...
              <Text style={styles.permissionText}>
                {mountError ?? '메이크업 추출을 위해 카메라 접근을 허용해주세요.'}
              </Text>
      ...
      <FloatingOverlayIconButton
        accessibilityLabel="메이크업 추출 촬영 닫기"
        onPress={onClose}
      ...
      <CameraCaptureControlRow
        ...
        centerSlot={
          <CameraCaptureButton
            accessibilityLabel="메이크업 추출 사진 촬영"
