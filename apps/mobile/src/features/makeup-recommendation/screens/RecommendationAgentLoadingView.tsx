...
import {Animated, Easing, Image, ScrollView, StyleSheet} from 'react-native';
import {
  Brush,
  ClipboardList,
  Clock3,
      ...
      <YStack style={styles.content}>
        ...
        <View style={styles.conversationPanel}>
          <ScrollView
            ...
            <YStack
              accessibilityLabel="AURA 메이크업 크루 대화"
...
function ReportPhotoScan({faceImageUri}: {faceImageUri?: string}) {
  ...
  const scanOpacity = scan.interpolate({
    ...
    outputRange: [0, 0.88, 0.88, 0],
  });
  const scanTransform = {
    transform: [
      {translateY: scan.interpolate({inputRange: [0, 1], outputRange: [12, 176]})},
    ],
  };

  return (
    <View
      accessibilityLabel="보고서 얼굴 사진을 확인하는 중"
      style={styles.scanFrame}>
      {faceImageUri ? (
        <Image resizeMode="cover" source={{uri: faceImageUri}} style={styles.scanImage} />
      ) : (
        <View style={styles.scanFallback}>
          <ImagePlus color={colors.textTertiary} size={iconSize.md} strokeWidth={1.8} />
          <Text style={styles.scanFallbackText}>얼굴 분석 사진을 불러오는 중이에요</Text>
        </View>
      )}
      <View style={styles.scanDim} />
      <Animated.View
        pointerEvents="none"
        style={[styles.scanBeam, {opacity: scanOpacity}, scanTransform]}
      />
      <Animated.View
        pointerEvents="none"
        style={[styles.scanLine, {opacity: scanOpacity}, scanTransform]}
      />
      <XStack style={styles.scanBadge}>
        <LiveDot />
        <Text style={styles.scanBadgeText}>사진 분석 중</Text>
      </XStack>
    </View>
  );
}
