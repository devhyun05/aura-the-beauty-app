  ...
  const palette: MakeupScenarioPalette = accentIds.has(id)
    ...
    : inkIds.has(id)
      ...
      : tone === 'premium'
        ? 'paper'
        : 'muted';
  const visualEmphasis = displayText.length <= 16 && (copyStyle === 'editorial' || copyStyle === 'character')
    ? 'featured'
...
export const MAKEUP_SCENARIOS: readonly MakeupScenarioPrompt[] = [
  scenario('must-look-beautiful', '중요한 날, 어떻게든 잘 나오는 룩', '중요한 날 가장 아름답고 자신 있어 보이는 메이크업', ['important', 'polished'], [], 'playful'),
  scenario('most-beautiful-self', '조용히 정돈된 프리미엄 밸런스', '나의 장점을 살린 균형 있고 아름다운 메이크업', ['balanced', 'premium'], [], 'premium'),
  scenario('commute-crush', '출근길 자주 마주치던 사람에게 먼저 인사하는 날', '출근길에 자연스럽지만 평소보다 매력적으로 보이는 메이크업', ['commute', 'romance'], ['occasion'], 'narrative'),
  scenario('five-minute-polished', '5분 만에 완성한 정교한 인상', '5분 안에 완성하지만 정교해 보이는 메이크업', ['quick', 'polished'], ['timeSkill'], 'playful'),
  scenario('trend-my-way', 'SNS에 저장만 해둔 그 룩, 이번엔 직접', '최근 메이크업 무드를 나에게 자연스럽게 적용한 룩', ['trend', 'personal'], ['mood'], 'premium', 'trend'),
  ...
  scenario('art-student', '느낌 좋은 미대생 전시 오프닝 룩', '색감과 질감에 취향이 느껴지는 힘 뺀 메이크업', ['art', 'effortless'], ['mood'], 'playful'),
  scenario('music-heiress', '부잣집 고명자식 음대생 룩', '단정한 클래식에 우아한 여유가 느껴지는 메이크업', ['classic', 'elegant'], ['mood'], 'narrative'),
  scenario('drama-comeback', '점 하나 찍고 완전히 달라진 서사', '이전과 확실히 달라 보이는 세련되고 선명한 메이크업', ['transformation', 'dramatic'], ['boldness'], 'narrative'),
  scenario('camera-first', '공항 출국 사진 레전드 룩', '사진과 조명에서 이목구비가 선명하게 살아나는 메이크업', ['photo', 'defined'], ['occasion'], 'playful'),
  ...
  scenario('neon-two-am', '을지로 새벽 네온을 닮은 룩', '네온 조명에서 색과 광택이 매력적으로 보이는 메이크업', ['night', 'neon'], ['occasion', 'mood'], 'premium'),
  ...
  scenario('wedding-balance', '예식장 조명까지 계산한 하객 룩', '격식 있는 자리에서 화사하고 균형 잡힌 메이크업', ['special', 'balanced'], ['occasion'], 'premium'),
  scenario('light-photo', '자연광과 플래시에 모두 강한 룩', '자연광과 사진에서 피부 표현과 색조가 조화로운 메이크업', ['light', 'photo'], ['occasion'], 'premium'),
  scenario('glasses-eyes', '안경 너머로도 살아나는 눈매', '안경테에 가리지 않고 또렷해 보이는 아이 메이크업', ['glasses', 'eye'], ['occasion'], 'playful'),
  scenario('concert-encore', '앵콜까지 살아남는 콘서트 룩', '열기와 긴 시간에도 포인트가 유지되는 콘서트 메이크업', ['concert', 'lasting'], ['occasion'], 'narrative'),
  scenario('festival-sunset', '새벽까지 살아남는 페스티벌 헤드라이너 룩', '낮부터 노을과 밤 조명까지 어울리는 페스티벌 메이크업', ['festival', 'sunset'], ['occasion'], 'narrative', 'trend'),
  ...
  scenario('hip-point', '성수동 감성 느좋 룩', '전체는 정돈하고 한 부위에 감각적인 포인트를 준 메이크업', ['hip', 'point'], ['mood'], 'playful'),
  ...
  scenario('saved-look', '음악프로그램 무대 접수 아이돌 룩', '온라인에서 저장하던 트렌디한 룩을 현실적으로 적용한 메이크업', ['saved', 'trend'], ['mood'], 'narrative', 'trend'),
  ...
  scenario('bookstore-owner', '연남동 독립서점 단골 룩', '차분한 색과 지적인 분위기가 느껴지는 메이크업', ['bookstore', 'calm'], ['mood'], 'narrative'),
  scenario('film-senior', '홍대 4번 출구 서브컬처 룩', '절제된 색감과 깊은 눈매가 인상적인 메이크업', ['cinema', 'moody'], ['mood'], 'narrative'),
  scenario('gallery-weekend', '이태원 해방촌 힙스터 룩', '힘을 뺀 피부와 감각적인 색 포인트의 주말 메이크업', ['gallery', 'weekend'], ['occasion', 'mood'], 'premium'),
];
...
const products = (prefix: string, accent: string): MakeupRecommendationProduct[] => [
  ...
  {id: `${prefix}-eye`, area: 'eye', brandName: '벨루어', productName: '음영 팔레트', shadeName: accent, reason: '룩의 주조색을 연결'},
  ...
  {id: `${prefix}-lip`, area: 'lip', brandName: '로제드', productName: '세럼 틴트', shadeName: accent, reason: '전체 룩의 포인트를 완성'},
];
