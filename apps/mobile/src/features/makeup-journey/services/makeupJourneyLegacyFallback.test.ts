...
expect(july17?.representativeReportId === 'latest', 'legacy calendar identifies its latest fallback report');
expect(july17?.representativeScore === 85, 'legacy calendar defaults representative score to latest');
expect(july17?.representativeThumbnailUrl === null, 'legacy calendar never labels a full-size URL as a thumbnail');
