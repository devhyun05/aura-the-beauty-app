...
type ARFilterStyleAdjustSelectedColor = Pick<FilterColorOption, 'hex' | 'label'>;

const AR_FILTER_CUSTOM_STYLE_FALLBACK_COLOR: ARFilterStyleAdjustSelectedColor = {
  hex: colors.white,
  label: '기본',
};
...
export function getARFilterStyleAdjustCameraMode(): 'live-camera' {
  return 'live-camera';
}

export function getARFilterStyleAdjustSelectedTabOpacity(): number {
  return SELECTED_TAB_BACKGROUND_OPACITY;
}

export function getARFilterStyleAdjustSelectedColor(
  colorOptions: readonly FilterColorOption[],
  selectedColorId: string,
): ARFilterStyleAdjustSelectedColor {
  return (
...
export function ARFilterStyleAdjustScreen({
  ...
  const [styleState, setStyleState] = useState<FilterStyleState>(getFilterStyleState());
  const selectedColor = getARFilterStyleAdjustSelectedColor(
    filter.colorOptions,
