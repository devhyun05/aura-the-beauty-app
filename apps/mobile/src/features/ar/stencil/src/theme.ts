...
// ── 간격·정렬 그리드 토큰 (4pt 그리드) ────────────────────────────────────────
// 파라미터 패널(레인 세그·탭·카드·슬라이더·룩 트리·축 컨트롤)의 여백/정렬을 한
// 스케일로 수렴시키는 단일 출처. 소비처는 임의 픽셀값 대신 이 토큰을 참조한다.
//  · SP        = 4pt 스텝(xs=라벨↔컨트롤, sm=반복 요소 간, md/lg=위계 간).
//  · PANEL_INSET = 패널 안 모든 줄이 시작하는 공통 좌측선. 카드 폭 산식(window-48)이
//                  paramPanel 마진(12)+본문 패딩(12)=24/측을 가정하므로 12로 고정.
//  · ROW_GAP   = 세로 리듬 기본(같은 위계 반복 요소 간).
//  · CONTROL_H = 칩/세그/버튼 공통 높이 — 파일 불문 동일(패딩 드리프트로 같은 줄
//                칩들이 몇 px씩 어긋나던 것을 고정 높이로 제거).
export const SP = { xs: 4, sm: 8, md: 12, lg: 16 } as const;
export const PANEL_INSET = 12; // 패널 좌우 공통 인셋(카드 폭 산식과 정합 — 스펙 16→실측 12)
export const ROW_GAP = 8; // = SP.sm
export const CONTROL_H = 28; // 4pt 그리드 정렬(스펙 34→실측 28: 현행 칩 높이대와 정합)
