...
export function ensureBrowTree(
  ...
  const leaves = visibleBrowLeaves(root);
  if (leaves.length === 1 && leaves[0].region === 'browStyle') return root;
...
export function patchBrowTree(
  ...
  if (
    ...
    BROW_REFERENCE_SHAPES.some(shape => shape.value === patch.shapeValue)
  ) {
    next = updateBrowSharedAxis(next, {browShape: patch.shapeValue});
    const selected = BROW_REFERENCE_SHAPES.find(
      shape => shape.value === patch.shapeValue,
    )!;
