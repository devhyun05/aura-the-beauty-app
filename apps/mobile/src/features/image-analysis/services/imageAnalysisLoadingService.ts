...
export const imageAnalysisLoadingSteps: readonly ImageAnalysisLoadingStep[] = [
  {
    id: 'face-map',
...
export function getImageAnalysisProgressState(
  ...
  const stepIndex = Math.min(
    Math.floor(progress * imageAnalysisLoadingSteps.length),
    imageAnalysisLoadingSteps.length - 1,
  );
  const progressPercent = Math.round(progress * 100);

  return {
    activeStep: imageAnalysisLoadingSteps[stepIndex],
    progress,
