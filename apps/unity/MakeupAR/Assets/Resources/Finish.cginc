                       ...
                       out float edgeSoft, out float grain, out float coverage, out float body)
{
    ...
    // 11 eyelinerLower — 0 펜슬(소프트+그레인) 1 스머지 파우더(페더 최대) 2 글리터
    // 리퀴드 3 리퀴드(무변조=선명, 기본). 이전 펜슬(-0.2=대비화)·스머지(0.35)는
    // 리퀴드와 구분이 안 보여(실기기) 페더를 크게 벌렸다.
    TEX_SEED(11, 0, 0.45, 0.6, -0.05, 0.15);
    TEX_SEED(11, 1, 0.9, 0.35, -0.2, -0.1);
    TEX_SEED(11, 2, 0.1, 0.15, -0.1, 0);
    TEX_SEED(11, 3, 0, 0, 0, 0);
    // 12 brow
