...
Shader "ARMakeup/FaceMakeup"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // 이마 띠 갭 처방: CY·AY 동반 +0.02 → 오벌 상단만 0.94→0.98로 올려 헤어라인쪽
            // 커버를 넓히고, 하단(CY−AY=0.02)은 그대로라 턱·목 과커버 없음. 실기기 튜닝 대상.
            #define FND_EDGE_CY 0.50  // 오벌 중심 y (실기기 튜닝 대상)
            #define FND_EDGE_AX 0.53  // 타원 x 반경 정규화 (실기기 튜닝 대상)
            #define FND_EDGE_AY 0.48  // 타원 y 반경 정규화 (실기기 튜닝 대상)
            ...
            // 콧볼 옆 갭 처방: 제외 반경을 콧구멍 개구부에 더 타이트하게(콧방울 옆 피부는
            // 파운데가 먹어야 함). RX↓가 핵심(옆으로 새던 제외를 콧구멍 안으로). 실기기 튜닝 대상.
            // 2차 축소(0.025→0.016·0.026→0.020): FEAT_FEATHER 0.30이 제외 타원을 반경의
            // 1.30배까지 부풀려(feat>0 스커트가 콧방울 옆 피부까지 감쇠) 1차 축소로도 갭이 남았다.
            // 코어(feat=1, d≤0.70) 반경 = RX·0.70 = 0.011로 실측 콧구멍 개구부는 덮되, 바깥
            ...
            //  넘어 얼굴 전체 커버가 튀고 채도가 뭉개진 "창백한 가면"이 됐다, 실기기. 홍조 커버는
            //  전역 부스트가 아니라 별도 접근 필요.)
            // 베이스 팩(#18)
