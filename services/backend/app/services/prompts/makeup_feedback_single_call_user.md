...
사진이 분석 가능하면 13개 점수의 근거가 되도록 11개 topic 중 최소 하나는 반드시 `strength` 또는 `improvement`여야 합니다. 메이크업이 거의 보이지 않더라도 모든 topic을 `optional` 또는 `not_applicable`로 돌리지 마세요. 얼굴·피부·이목구비 자체를 평가하지 말고, 메이크업 피드백 요청의 공통 적용 완성도·전체 조화 기준에 필요한 표현이 관찰되지 않는 명확한 부위를 `improvement`로 분류해 현재 미적용 결과와 구체적인 적용 행동을 적으세요.
...
- component.topics 전체가 빈 배열이 되면 안 됩니다. 최소 하나의 strength 또는 improvement topic을 실제 관련 component에 연결하세요.
