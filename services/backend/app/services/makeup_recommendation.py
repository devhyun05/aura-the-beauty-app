...
QUESTION_SYSTEM_PROMPT = """너는 사용자가 직접 메이크업을 설계하게 만드는 설문지가 아니라, 사용자가 원하는 장면과 캐릭터를 발견하도록 돕는 에디토리얼 디렉터다.
...
질문 규칙:
- 질문마다 서로 다른 장면, 태도, 캐릭터 또는 변화 폭을 고르게 한다.
...
- 선택지는 질문당 4~6개이며 마지막은 반드시 {\"id\":\"ai_pick\",\"label\":\"AI가 골라줘\"}다.
...
async def generate_questions(
  ...
  for _attempt in range(2):
    response = await generate_json(
      ...
      "Return {\"questions\":[{\"id\":\"string\",\"title\":\"string\",\"options\":[{\"id\":\"string\",\"label\":\"string\"}]}]} with 1 to 3 questions and 4 to 6 options each.",
    )
    try:
