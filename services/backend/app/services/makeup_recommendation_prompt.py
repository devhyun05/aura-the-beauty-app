...
사용자가 적은 장면·일정·직업·역할·테마·원하는 인상을 메이크업 맥락으로 해석한다. situationIntent에는 원문의 핵심 맥락을 간결하게 보존하고, 특정 시간이나 장소가 없다는 이유로 입력을 거절하지 않는다.
...
자유 입력의 situationIntent는 AI가 원문에서 해석한 핵심 맥락이므로 같은 내용을 다시 묻지 않는다. normalizedCustom의 desiredImpression과 constraints에 있는 준비 시간·지속력·환경·사진 우선순위·안전 회피 조건도 이미 답한 선택 정보로 간주한다.
...
치크와 립은 혈색 계열(핑크·로즈·코럴·피치·베이지·모브·베리) 안에서 고른다. 민트·그린·블루·퍼플·옐로우 같은 비혈색 계열은 상황·키워드·답변이 무대·화보·페스티벌·파티 같은 컨셉 컬러를 명시적으로 요구할 때만 쓴다. 붉은기 보정용 그린·민트 코렉터는 base 가이드의 보정 단계에서만 다루고 치크·립 색으로 옮기지 않는다.
퍼스널컬러는 색을 고르는 팔레트가 아니라 워스트 컬러 회피 필터다. 색 계열·무드(뮤트·비비드·라이트·딥)·톤 방향(웜·쿨)은 모두 상황·답변·컨셉이 정한다 — 상황에 어울리는 메이크업이 웜톤이면 쿨톤 시즌 사용자에게도 웜톤을 추천하고, 여름 라이트 사용자라도 뮤트가 어울리는 상황이면 쿨톤 뮤트를 추천한다. 퍼스널컬러의 역할은 두 가지뿐이다: 상황·컨셉이 톤 방향을 정하지 않았을 때 같은 계열 안에서 언더톤 기본값을 고르는 것(옐로우 컨셉이면 쿨톤은 레몬, 웜톤은 버터 옐로우), 그리고 그 사람에게 확연히 충돌하는 워스트 색만 피하는 것. 퍼스널컬러 시즌 팔레트의 대표색을 그대로 부위 색으로 가져오거나, 시즌 타입을 이유로 상황에 맞는 무드·계열·톤을 좁히지 않는다.
...
def build_custom_normalization_prompt(text: str) -> str:
  return (
    ...
    "형태로 반환하라. situationIntent에는 장면·일정·직업·역할·테마·원하는 인상 중 원문이 제공한 핵심 메이크업 맥락을 넣어라. 시간이나 장소가 없어도 입력의 의미를 그대로 해석하라. "
