...
_ANALYSIS_SEC_PRESCRIPTION = (
  ...
  "makeupColors는 일상 룩까지 모든 룩에 공통 적용되는 기본값이니 brow는 사진에서 관찰한 "
  "실제 모발·눈썹 색과 조화되는 색을 기본으로 해 — 자연 흑발·갈색 모발이면 브라운·애쉬·토프 "
  "계열에서 퍼스널컬러의 웜/쿨 축만 반영하고, 핑크·블루·모브 같은 특이 염색 모발이면 그 "
  "모발색과 어울리는 톤을, 탈색 눈썹이면 밝은 블론드·베이지 톤을 써. 실제 모발·눈썹 색 근거 "
  "없이 퍼스널컬러만으로 모브·플럼 같은 패션 컬러 브로우를 만들지는 마 — 그건 개별 룩이 "
  "컨셉에 따라 결정할 영역이야. "
