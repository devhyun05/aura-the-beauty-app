  ...
  return success(
    {
      ...
      "externalDataReady": readiness["eligibleProducts"] > 0,
      "readiness": readiness,
    },
...
async def get_seasonal_product_recommendations(
  ...
  data = await get_seasonal_recommendations(db, settings, locale=locale, limit=limit)
  ...
  source = "live-seasonal-discovery" if collection.get("isLive") else "editorial-seasonal-v1"
...
async def get_personalized_product_recommendations(
  ...
  response.headers["Cache-Control"] = "private, no-store"
  return success(data, {"source": "consented-engagement", "algorithmVersion": "engagement_decay_v1"})
...
async def get_cohort_product_recommendations(
  ...
  user = await ensure_user(db, auth)
  await enforce_product_rate_limit(db, user_id=user["id"], scope="recommendation", limit=settings.product_recommendation_rate_limit_per_minute)
  data = await get_cohort_recommendations(db, settings, user_id=user["id"], limit=limit)
  response.headers["Cache-Control"] = "private, no-store"
  return success(data, {"source": "privacy-thresholded-cohort-v1"})
...
async def search_products(
  ...
  if not db.is_connected:
    return success({"status": "unavailable", "searchRequestId": str(request_id), "items": [], "nextCursor": None})
  user = await ensure_user(db, auth)
  ...
  return success(
    ...
    {"source": "trusted-catalog", "catalogVersion": "catalog_v2"},
  )
