...
The init command records the current versioned schema marker in
`schema_migrations`, so repeated runs are skipped by default while pending
forward migrations are still applied. The seed command records its own
versioned marker the same way.
...
Face reports are text-only and never create recommendation images. The image
generation settings above apply only to the separate personalized makeup
recommendation workflow. That workflow resizes the source before OpenAI image
editing and the generated JPEG before S3 upload.

The staged S1 + Face3D analysis is opt-in while it is being evaluated:

```env
FACE_ANALYSIS_V2_ENABLED=true
FACE_ANALYSIS_STAGE_TIMEOUT_SECONDS=130
...
To exercise engagement- and cohort-backed recommendations in an isolated
local demo, opt in explicitly in `.env` (the checked-in defaults stay off for
...
Even before the local demo user has enough engagement, cohort history, or a
saved AR look, the recommendation APIs return catalog-backed popular fallbacks
so the product page is not empty. A new user is expected to have no liked
products; exact AR color/texture matching starts after they save an AR look.

For a physical phone, point `apps/mobile/.env` at the Mac/PC LAN address rather
than `localhost`:
...
See `docs/backend/LOCAL_BACKEND_VERIFICATION.md` for the product endpoint
checks and expected fallback behavior.
