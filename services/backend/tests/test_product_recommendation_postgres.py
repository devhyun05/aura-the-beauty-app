...
async def test_product_recommendation_v2_contracts_in_postgres(
  ...
  try:
    ...
    settings = Settings(
      ...
      product_cohort_min_item_support=5,
    ...
    assert cohort_key == "preference-v3:other"
    merged_cohort = await get_cohort_recommendations(
      db,  # type: ignore[arg-type]
      settings.model_copy(update={"product_cohort_experiment_percent": 100}),
      user_id=user_id,
      limit=10,
    )
    assert merged_cohort["status"] == "ready"
    assert merged_cohort["cohortStatus"] == "insufficientData"
    assert merged_cohort["fallback"]["reason"] == "MERGED_COHORT_NOT_SIMILAR_ENOUGH"
    assert merged_cohort["items"][0]["reasonCodes"] == ["POPULAR_FALLBACK"]

    cohort_key = "preference-v3:color-cool"
    await connection.execute(
      "update product_color_cohort_memberships set cohort_key=$2 where user_id=$1",
      user_id,
      cohort_key,
    )
    await connection.execute(
      ...
      ) values ($1,$2,'broad_report_preference_v3',1,now()+interval '1 day')
      """,
      creator_id,
      cohort_key,
    )
    ...
    assert default_cohort["status"] == "ready"
    assert default_cohort["cohortStatus"] == "insufficientData"
    assert default_cohort["fallback"]["reason"] == "COHORT_BELOW_PRIVACY_THRESHOLD"
    assert default_cohort["items"][0]["reasonCodes"] == ["POPULAR_FALLBACK"]
