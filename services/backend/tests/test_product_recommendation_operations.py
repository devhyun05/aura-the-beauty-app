...
def test_public_feature_status_is_honest_without_a_catalog_database() -> None:
  ...
  assert data["externalDataReady"] is True
...
def test_seasonal_public_response_supports_etag_with_external_catalog_fallback() -> None:
  client = TestClient(create_app(Settings(database_url=None)))
...
def test_search_returns_server_owned_external_catalog_without_db() -> None:
  client = TestClient(create_app(Settings(database_url=None)))
  ...
  assert {item["externalSource"] for item in response.json()["data"]["items"]} == {"auradin_catalog"}
...
async def test_cohort_refresh_merges_rare_buckets_and_caps_contribution() -> None:
  ...
  assert all(args[1] == "preference-v3:other" for _, args in inserts)
